...
#par[
  在视觉感知层，系统基于 YOLOv8-OBB 旋转目标检测模型识别面包板上 9 类电子元器件，通过 OpenVINO 部署于 iGPU 实现毫秒级推理，采用 Image-only 高分辨率图片分析架构，支持 1-3 张图片多图 IoU 融合以消除遮挡盲区。系统提出视觉引脚-孔洞占用检测算法（Median+MAD 鲁棒统计 + Sigmoid 评分），结合几何候选约束回退策略，实现元件引脚到面包板孔位的精确映射。在逻辑推理层，系统设计了四级鲁棒面包板校准管线与基于 NetworkX 的电路拓扑构建引擎，采用 VF2++ 子图同构与 GED 图编辑距离实现多级电路验证，支持 IC 多引脚 Hub 节点建模与 4 轨道电源轨学生标注。在认知层，系统部署三级降级 LLM 引擎（云端 API → Qwen2.5-1.5B INT4 NPU 推理 → 规则模板），结合 ChromaDB RAG 检索增强与 OCR 芯片自动识读，确保离线环境下仍可提供智能问答。
]
...
*（三）视觉引脚-孔洞占用检测（PinHoleDetector）— 核心创新。*

YOLO 检测框仅描述元件外观轮廓，但确定引脚具体插入面包板哪个孔洞才是电路建模的关键前提。传统方法依据几何最近邻将引脚像素坐标映射到最近孔位，由于 OBB 边界估计存在 1–3 行误差，在密集元件场景下产生大量误匹配（单场景假阳性可达 75–214 个）。本系统提出一种基于视觉特征的孔洞占用检测方法，从图像层面直接判断哪些孔洞被引脚占用。

核心思想：面包板空孔呈现为深色圆洞（低亮度、高中心-边缘对比度），当元件引脚插入后，孔洞的视觉特征发生显著变化——中心亮度升高（金属反光取代暗空洞）、对比度降低、梯度能量增强。算法管线如下：

*Step 1 — 局部 Patch 提取。*对校准后的每个孔洞位置，通过逆透视变换映射回原始高分辨率图像，提取中心 ±R 像素的局部图像块。

*Step 2 — 多维特征提取。*对每个 Patch 计算四维特征向量：中心 5×5 区域平均灰度（center\_intensity）、中心灰度与边缘环灰度之差（contrast）、Sobel 梯度在中心区域的能量（gradient\_energy）、RGB 三通道标准差均值（color\_std）。

*Step 3 — Median+MAD 鲁棒统计建模。*基于"大多数孔洞为空"的先验假设，对全图孔洞特征计算中位数（Median）与中位绝对偏差（MAD），建立空孔基线模型。MAD 相比标准差对离群值（即被占用的孔洞）具有天然免疫力，无需预先标注训练数据。

*Step 4 — Sigmoid 占用评分。*计算每维修正 Z-score：$z = |f - tilde(f)| \/ (M A D times 1.4826)$，经绝对特征门控后加权融合（对比度 0.40 + 亮度 0.25 + 梯度 0.20 + 颜色 0.15），通过 Sigmoid 函数 $sigma(x) = 1\/(1 + e^(-(x - 3)))$ 映射为 0–1 占用概率。

*Step 5 — 导线阴影聚簇过滤。*横跨面包板的导线会在下方孔洞投射连续阴影，被误判为占用。系统检测同行/同列 ≥4 个连续高分孔洞，判定为阴影干扰并抑制中间节点。

*Step 6 — 双策略引脚定位。*对非导线元件优先使用视觉占用结果，以自适应搜索半径（按元件类型调整）+ 主轴对齐约束过滤候选孔洞，经导通组短路惩罚与合理跨度奖励选出最佳引脚对。仅在视觉检测无法给出可靠结果时回退至几何 Top-K 候选约束匹配。
...
#figure(
  rect(width: 90%, height: 4.5cm, stroke: 0.5pt + gray)[
    ...
    #set par(first-line-indent: 0em)
    #text(size: 9pt, fill: gray)[
      【图片占位】引脚定位算法流程图（本项目核心创新）\
      建议内容：上半部分为 6 步视觉占用检测管线（Patch 提取 → 特征计算 → MAD 建模 → Sigmoid 评分 → 阴影过滤 → 引脚匹配）\
      下半部分为双策略分支（视觉优先 / 几何回退）\
      建议附加一组面包板局部区域的占用评分可视化（热力图：红=占用，绿=空）
    ]
  ],
  caption: [视觉引脚-孔洞占用检测算法流程（核心创新）],
) <fig:pinhole>

*实验效果。*在 3 个真实面包板场景中，视觉占用检测相比纯几何方法显著降低引脚定位假阳性，如@tab:pinhole-result 所示。
...
#figure(
  ...
  caption: [引脚定位假阳性对比（几何方法 vs 视觉占用检测）],
) <tab:pinhole-result>
...
*（1）视觉引脚-孔洞占用检测（核心创新）。*首次将 Median+MAD 鲁棒统计应用于面包板孔洞分析，结合 Sigmoid 评分与绝对特征门控实现引脚插入位置的精确判定，配合几何约束回退形成双策略互补体系。实测假阳性降低 17%–76%。
...
#figure(
  table(
    ...
    [引脚定位假阳性], [降低 17%–76%], [视觉占用检测 vs 纯几何方法],
...
#figure(
  table(
    ...
    [3 月 5 日—15 日], [软件开发], [四层架构 + PinHoleDetector + RAG + GUI],
