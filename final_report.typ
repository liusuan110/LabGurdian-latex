...
#figure(
  diagram(
    ...
    // L5
    node((0, 6), text(9pt)[#strong[L5] · Gemma3-4B polish (intent-aware)]),
...
+ *Mac/云端有网*：从 HuggingFace 下载 Gemma3 ckpt → OpenVINO INT4 量化 → 打包 IR 文件夹（~2GB）；
+ *U 盘 / SD 卡 → 板子*：把 IR 包 + OpenVINO runtime 离线 wheel + 项目代码一并拷贝；
+ *板子上*：解压、安装 wheel、设置 `.env: AGENT_LLM_PROVIDER=openvino_genai_text`、启动 `uvicorn`；
+ *全程不需外网*。
...
+ *端侧 OpenVINO 部署落实*：完成 Gemma3-4B INT4 量化、DK-2500 实测延迟数据采集（任务 CADx-9）；
+ *Edit Script 生成器 (CADx-T3)*：基于 Template 模板锚定的"接线差异 → 步骤化修改建议"自动生成，从"指出问题"升级到"指导修复"；
+ *Retrieval-Augmented Distillation (RAD)*：以 AnalogSeeker-32B 为教师，使用本地结构化 KB 作为 ground truth 蒸馏 Gemma3-4B 至教学专用，减小教师错误放大风险；
