...
- #text(
    "人工修正：交互端支持端口标注、孔位修正、网络角色指定、集成芯片标注和引脚极性选择，提交后重新执行拓扑重构与参考比较。",
  )
...
#par[#text(
  "LabGuardian 定位为面向电子实验教学场景的边缘智能助教系统。与在虚拟环境中验证电路原理的通用仿真软件不同，本系统以学生实际搭建的面包板照片为输入，将图像中的元件、引脚、孔位与电气连接重建为可验证、可审计的结构化事实，进而与参考电路进行逻辑比较，输出错误位置、错误成因、风险等级与修改建议。其方法学意义在于将高度依赖经验的实验巡检过程形式化为可复现、可解释的诊断流程，从而降低教师的重复排错负担，并为学生提供搭建过程中的即时反馈。",
)]
...
#par[#text(
  "系统的必要性源于前述教学痛点与现有方案的局限：真实实验的排错高度依赖教师经验且重复性强，而通用仿真、纯规则比较与端到端大模型均无法同时满足真实接线感知、可解释诊断与边缘安全运行的要求。LabGuardian 以结构化事实链为核心，正是为弥合“原理图—实物接线—故障成因”三重映射之间的断层而设计。",
)]
...
#par[#text(
  "如果学生或教师发现视觉映射结果不准确，交互端可以进入网表模式进行人工修正。用户可以修正引脚孔位，为输入/输出端口打标，设置网络角色，或补充三极管和集成芯片的人工标注。交互端随后调用人工修正重算接口，把映射阶段的元件与修正信息一并提交给服务端。服务端不再重跑视觉检测，而是从修正后的元件重新执行拓扑重构、参考验证和语义分析。这种设计兼顾 AI 自动识别与教学现场的人机协同，避免视觉阶段偶发错误导致整个系统不可用。",
)]
...
#par[#text(
  "在上述选型基础上，系统设计遵循四项原则。第一，事实链优先。视觉模型只产生候选事实，最终教学结论必须经过板型规则、拓扑分析和验证模块约束。第二，接口契约优先。各阶段的输出，以及结构化网表、诊断报告、运行证据和上下文包，都以统一的结构化格式作为协作边界，避免交互端、服务端或智能体私自猜测。第三，人机协同优先。系统允许人工端口标注和孔位修正，教师与学生可以把 AI 输出修正为可信事实后再重算。第四，边缘可观测优先。运行结果保留模型路径、推理参数、板型定义、版本号和阶段耗时，后续可与遥测数据一起用于验收展示和实验分析。",
)]
...
#par[#text(
  "本系统的核心工程方法是将“看图诊断电路”分解为若干可解释的事实转换步骤，而非以单一端到端大模型直接给出结论：组件检测负责元件实例、引脚检测负责引脚候选、孔位映射负责孔位吸附、拓扑重构负责导通网络、参考比较负责逻辑比较，智能体仅负责解释。这一结构化事实链使每一步均可被独立测试、独立可视化与独立修正，错误定位也因此更为具体，并从根本上抑制了端到端模型难以审计、易于幻觉的风险。本章聚焦事实链的前段，即从图像到引脚与孔位的视觉感知与映射环节；电气重构与逻辑诊断见第四章，智能解释与人机协同见第五章。",
)]
...
#par[#text(
  "全图引脚检测阶段是从元件框级识别进入引脚级事实的关键。当前主路径不再先裁剪单个元件区域再分别识别引脚，而是对主视图整图执行 YOLO-Pose 关键点检测，再根据类别和边界框的几何关系，将检测到的引脚关联回对应元件。这种方式能减少裁剪误差和局部上下文丢失，也更契合未来多视角融合的证据整理需求。",
)]
...
#par[#text(
  "拓扑重构的核心产物——结构化网表——是贯穿后续比较、解释与统计的关键数据结构。它并非文字形式的网表，而是一个可被交互端、验证模块与智能体共同消费的结构化对象，核心包含板型标识、元件列表、网络列表、节点索引与板型拓扑。其中每个元件记录元件编号、类型、封装、子类型、极性、方向、对称组、引脚列表与置信度；每个网络记录网络编号、成员节点、成员孔位与电源角色；节点索引保存导通节点到物理孔位的映射，板型拓扑则保存完整的节点—孔位映射与电源轨配置。借助这一结构，交互端不仅能判定某引脚归属哪一网络，还可在用户选中某网络时高亮其全部相关孔位。",
)]

== #text("4.2 参考比较与诊断报告")

#par[#text(
  "参考比较阶段只接受逻辑参考电路，不再支持孔位级或物理点位级的参考对比。该设计契合教学场景：学生实际摆放的孔位可以不同，但只要电气逻辑等价，电路就应判定为正确；反之，即使孔位看似接近，只要逻辑网络接错，就应输出诊断。参考比较会把参考电路和当前网表分别转换为图模型，再进行逻辑结构比较。",
)]
...
#par[#text(
  "感谢学院与实验中心提供的实验场地、面包板与电子元器件等实验条件，以及边缘计算开发平台的使用支持，使系统得以在真实硬件上完成部署与性能验证。",
)]
...
#par[#text(
  "最后，感谢所有关心和支持本项目的师长、同学与家人。文中难免存在疏漏与不足，恳请各位老师与同行批评指正。",
)]
