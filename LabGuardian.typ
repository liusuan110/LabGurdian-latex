...
#figure(
  image("pictures/图2-1.png", width: 50%),
  caption: [系统总体架构图],
) <fig:arch>
...
#figure(
  image("pictures/图2-2.png", width: 90%),
  caption: [主业务流程图],
) <fig:pipeline>
...
#table(
  ...
  [#text("交互端")], [#text("React 19、Vite、TypeScript、lucide-react")], [#text("适合构建交互式单页应用和课堂看板")],
  [#text("边缘部署")],
...
#par[#text(
  "本系统的核心工程方法是将“看图诊断电路”分解为若干可解释的事实转换步骤，而非以单一端到端大模型直接给出结论：S1 负责元件实例、S1.5 负责引脚候选、S2 负责孔位吸附、S3 负责导通网络、S4 负责逻辑比较，智能体仅负责解释。这一结构化事实链使每一步均可被独立测试、独立可视化与独立修正，错误定位也因此更为具体，并从根本上抑制了端到端模型难以审计、易于幻觉的风险。本章聚焦事实链的前段，即从图像到引脚与孔位的视觉感知与映射环节（S1 至 S2）；电气重构与逻辑诊断（S3 至 S4）见第四章，智能解释与人机协同见第五章。",
)]
...
#par[#text(
  "S2 阶段将 S1.5 输出的有序引脚预测映射到面包板的物理孔位和电气导通节点。它首先尝试用主视图对面包板校准器进行校准；若校准失败，则回退到合成网格，并在校准模式中显式标记。随后，系统为每个引脚构造观测记录，结合关键点坐标、可见性、分数、来源和投影信息生成候选孔位，再依据板型规则把孔位解析为对应的导通节点。",
)]
...
#par[#text(
  "孔位吸附质量由关键点到孔位的距离和网格间距共同决定，吸附置信度采用 1-(d/pitch)² 的二次衰减形式计算。模型置信度高但关键点明显偏离孔位的结果会被自动降权，低于阈值时记入歧义原因。对于两脚轴向元件、电解电容、跳线和电位器，S2 还引入了引脚配对选择与几何先验，防止两个引脚被独立吸附到不符合元件物理形态的孔位。",
)]
...
#par[#text(
  "S4 输出结构化诊断报告。每条诊断项包含错误类型、类别、严重程度、说明文字、建议动作、证据引用，以及涉及的元件、引脚、当前孔位与目标孔位、当前节点与目标节点、期望值与实际值等字段。常见的错误类型包括引脚悬空、导线端点未连接、元件两端落在同一网络造成短路、电源轨短路、出现意外的网络桥接、孔位接错、极性接反、元件缺失、引脚缺失、发光二极管缺少限流电阻等。交互端可借助证据引用和高亮协议显示具体的错误位置。",
)]
...
#par[#text(
  "诊断智能体并非只服务于一种问题，而是统一编排四类教学意图：面向“电路哪里错、怎么改”的诊断（diagnostic）、面向“这个原理或公式是什么”的概念讲解（concept_tutor）、面向“这一步该怎么测、怎么接”的操作指导（lab_guidance），以及二者兼有的混合问答（mixed）。系统先由一次轻量意图判定（大模型单次分类，失败时回退关键词规则）确定意图，四类意图随后走同一张状态图，仅按意图切换三件事：可用工具白名单、回答校验规则与回答模板。例如诊断意图按错误族挑选网表追踪、板型查询等工具并要求引用错误码与证据；概念意图以教学概念库为主源并要求标注概念标识；操作指导意图则必须给出编号步骤并前置断电等安全提示。这样既复用同一套推送式上下文与确定性校验机制，又让不同类型的教学问题得到结构恰当的回答，而无需为每类问题各维护一条独立链路。",
)]
...
#par[#text(
  "交互界面采用 React、Vite 与 TypeScript 构建单工位完整诊断流程。它的职责不是承担复杂业务规则，而是把服务端生成的结构化事实清晰地呈现给学生与教师。典型使用主线为：上传面包板图片，执行流水线诊断，依次展示 S1 至 S4 的事实链、元件、引脚、孔位与网表，再调用智能体给出诊断解释与修改建议。界面按访问服务端的接口封装、约束数据结构的类型定义、上传与画布标注等通用组件，以及负责状态管理与主流程编排的诊断流程模块组织。",
)]
...
#par[#text(
  "为系统刻画蒸馏带来的质量增益，项目设计了如下统一评测对比：在同一套教学评测集上横向比较未蒸馏基座、本系统的蒸馏学生模型、此前的 4B 级基线模型，以及作为质量上限的教师模型，指标涵盖引用准确率、禁用内容命中率、安全前置覆盖率与中立模型评判胜率，并附板上推理吞吐。下表为该对比的结构，质量类指标待联合 Gold Set 评测集与中立评判产出后填入，当前仅就已实测项先行标注。",
)]
...
#figure(
  table(
    ...
    inset: 6pt,
    stroke: 0.5pt,
    [#text("模型")], [#text("引用准确率")], [#text("禁用命中率")], [#text("安全前置覆盖")], [#text("评判胜率")], [#text("板上吞吐")],

    [#text("base Qwen2.5-1.5B（无蒸馏）")], [#text("待评测")], [#text("待评测")], [#text("待评测")], [#text("待评测")], [#text("约 23 tok/s（同量级）")],
    [#text("学生模型（SFT+INT4，本系统）")], [#text("待评测")], [#text("待评测")], [#text("待评测")], [#text("待评测")], [#text("约 23 tok/s（实测）")],
    [#text("gemma-3-4b（旧基线）")], [#text("待评测")], [#text("待评测")], [#text("待评测")], [#text("待评测")], [#text("文本后端无法加载")],
    [#text("教师（DeepSeek-V3 / Qwen3-32B，上限）")], [#text("待评测")], [#text("待评测")], [#text("待评测")], [#text("（参照）")], [#text("云端/大模型，仅作上限")],
  ),
  caption: [蒸馏学生模型与基座、旧基线、教师模型的质量对比结构（质量类指标待联合 Gold Set 评测与中立评判填入；板上吞吐为已实测项）],
) <fig:quality-compare>

#par[#text(
  "需要说明的是，本节给出的板端性能、内存与能耗均为部署态（iGPU、贪心解码、固定生成长度）的真机实测；而回答质量的评测目前仍限于小规模冒烟与少量典型场景的对比，尚未在完整测评集上给出准确率、引用正确率与胜率等量化指标。因此本节结论可概括为：从蒸馏到导出再到端侧运行的链路已在 DK-2500 上跑通，并在功耗与内存预算内可用，但学生模型回答质量的系统量化仍在进行；其质量边界见 7.2 节，完整测评集与多教师质量对齐计划见 7.5 节。",
)]
...
#figure(
  table(
    ...
    [#text("回归测试")], [#text("典型参考电路与错接样例")], [#text("逻辑正确率、错误类型一致性、建议动作稳定性")],
    [#text("交互端测试")],
    [#text("上传、结果展示、人工修正、智能体轮询")],
    [#text("状态流转、错误提示、按钮禁用、高亮目标一致性")],

    [#text("集成测试")], [#text("交互端→服务端→智能体端到端")], [#text("完整链路可运行、诊断结果与智能体回答可解析")],
    [#text("边缘测试")],
...
#par[#text(
  "在评测口径上，视觉环节将元件检测、引脚检测与孔位映射分开统计：元件检测看 mAP、漏检率与误检率，引脚检测看关键点误差、可见性分类与有序引脚的顺序正确率，孔位映射看首选命中准确率、前 k 候选覆盖率与低置信召回率，并在遮挡、反光、强弱光与倾斜样本上比较单视图与多视图的差异。拓扑与诊断环节以电路级任务为单位，在反相运放、积分器、求和放大器与三极管差分对等模板上构造正确、缺线、错孔、短路、极性反接、缺保护电阻等场景，统计逻辑判定准确率、错误类型命中率、错误定位准确率与误报漏报率。第四章给出的模板匹配 6/7 通过率与图学习辅助诊断的实测指标，即来自这一评测体系。",
)]
...
#par[#text(
  "系统的主要风险集中在四个方面。视觉识别面临遮挡、反光、拍摄角度、类别不平衡与关键点误差，应对方式是持续扩充真实样本、对低置信与多视图冲突样本建立人工复核、并把决定性视图与吸附距离可视化到交互端。拓扑比较依赖参考电路与端口标注，定义不准或标注缺失可能导致误判，应对方式是为参考电路建立审查流程、运行前强制检查电源轨与必要端口、并提供网表比较调试接口供教师验证。",
)]
...
#table(
  ...
  stroke: 0.5pt,
  [#text("术语")], [#text("含义")],
  [#text("物理孔位")], [#text("面包板上每个插孔的物理编号，如 A12、LP31")],
  [#text("导通节点")], [#text("面包板静态导通节点编号，由板型规则根据物理孔位解析")],
  [#text("电气网络")], [#text("动态电气网络编号，由元件和导线连接合并后生成")],
  [#text("结构化网表")], [#text("当前主链路的结构化网表格式，包含元件、引脚、网络和板型拓扑")],
  [#text("逻辑参考电路")], [#text("教师或参考模板提供的逻辑参考电路格式")],
  [#text("诊断报告")], [#text("参考比较（S4）输出的结构化诊断报告，包含错误类型、证据和建议")],
  [#text("运行证据")], [#text("智能体从课堂状态抽取的当前运行证据")],
  [#text("上下文包")], [#text("上下文管理机制编译后的最小上下文，包含事实、工具、规则和证据引用")],
  [#text("板型规则")], [#text("描述面包板孔位、导通节点、电源轨和别名的板型定义")],
  [#text("孔位吸附")], [#text("将视觉关键点吸附到最近面包板孔位并计算置信度的过程")],
)
...
#table(
  ...
  stroke: 0.5pt,
  [#text("模块")], [#text("说明")],
  [#text("视觉感知模块")], [#text("完成元件检测、引脚关键点识别和多视图候选证据生成")],
  [#text("孔位映射模块")], [#text("将图像关键点吸附到面包板孔位，并映射为电气导通节点")],
  [#text("拓扑重构模块")], [#text("基于板型规则和导线连接关系生成结构化网表与拓扑图")],
  [#text("参考比较模块")], [#text("将当前电路与逻辑参考电路进行图结构比较，输出诊断报告")],
  [#text("智能体诊断模块")], [#text("基于运行证据、知识库和确定性工具生成可解释反馈")],
  [#text("交互展示模块")], [#text("提供上传、结果展示、错误定位、人工修正和对话交互能力")],
  [#text("边缘遥测模块")], [#text("采集 CPU、内存、iGPU、NPU 和流水线阶段状态，用于性能观测")],
)
