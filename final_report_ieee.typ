...
本方案与"NPU 跑 LLM"主流话术相反：实测发现 OpenVINO 2026.1 vpux 编译器对 Gemma3 `q_proj` 矩阵乘有已知 issue (Channels 0 != 20)，Qwen2.5-7B INT4 在 7.3 GB RAM 上编译时 peak ~8 GB 触发 OOM，唯一稳定的 Qwen2.5-1.5B INT4 在 NPU 反而比 CPU 慢 (14 vs 41 tok/s)。NPU 真正用武之地是固定 shape 视觉 CNN。

== NPU Vision Acceleration (Measured)

#figure(
  image("pictures/cadx/power_timeseries.pdf", width: 100%),
  caption: [YOLOv8s-pose INT8 RAPL power timeseries on DK-2500 (turbostat 0.25 s sampling). Three colored bands = CPU/iGPU/NPU sustained 15 s each. CPU peak 30.9 W (cores carry it), iGPU 10.8 W total (GFX 4.4 W), NPU 10.0 W *with CPU cores and iGPU both idle*—enabling true heterogeneous parallelism with concurrent LLM inference on iGPU.],
) <fig:power-ts>
...
#figure(
  tlt(
    ...
    header: ([Device], [Prec], [Mean(ms)], [P99(ms)], [img/s], [Energy/inf]),
    rows: (
  ...
  kind: table,
  caption: [YOLOv8s-pose 6-config benchmark on DK-2500 (60--1153 inferences each). NPU INT8 dominates on all axes—latency, throughput, and energy efficiency. NNCF Fast Bias Correction over 144 calibration images (5 real breadboard photos × 8 augments + training-set batch composites) yields 50\% model size reduction (22 MB → 11 MB) with no measurable accuracy loss.],
) <tab:yolo-bench>

NPU INT8 leads CPU INT8 by *7.1× lower energy/inference* (114 mJ vs 814 mJ) and *12.1× better perf-per-watt* (18.1 vs 1.5 ips/W). Versus iGPU INT8, NPU is *1.5× more energy-efficient* and *1.6× higher perf/W*. Critically, the timeseries shows NPU operation leaves CPU cores and iGPU completely idle, enabling concurrent LLM inference and graph analysis—end-to-end latency becomes $max(t_"vision", t_"LLM") + t_"CPU"$ rather than the sum, compressing a serial 3.5 s pipeline to a parallel 2.5 s pipeline.
...
+ *OpenVINO 2026.1 NPU plugin 对 Gemma3 / 大尺寸 LLM 仍不成熟*——vpux 编译器对 Gemma3 `q_proj` 矩阵乘有 issue（Channels 0 != 20）、Qwen2.5-7B INT4 在 7.3 GB RAM 上 OOM；本项目实测后改用 NPU 跑 YOLOv8s-pose 视觉（见 #ref(<tab:yolo-bench>)）、GPU 跑 LLM；
