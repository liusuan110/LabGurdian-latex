...
数据集采集自真实面包板实验照片，覆盖一阶 RC、共射放大、差分对、UA741 反相/积分/加法等六类教学拓扑，并刻意纳入多角度、不同光照与导线遮挡等真实变体。标注采用“组件级边界框 + 引脚级关键点”的双层结构：每个元件实例除给出类别与边界框外，还以关键点形式标注其引脚位置（关键点张量形状为 3×3，即每实例至多 3 个引脚、每个引脚记录横纵坐标与可见性），从而把识别精度从元件级下沉到引脚级——这正是常规目标检测数据集所不具备、却为电气诊断所必需的标注粒度。该数据集由项目自行拍摄并标注的真实面包板照片构成，前期累计采集规模超过千张；在最终用于本轮 YOLOv8s-pose 训练的合并数据集中，共保留 809 张样本，其中 train/val/test 分别为 566/162/81。由于全部取自真实实验台、并刻意涵盖多角度、不同光照与导线遮挡，其样本本身即贴近课堂实拍的噪声分布，无需依赖合成增强。数据集主要规格如#ref(<tab:dataset>)所示。
...
#figure(
  三线表(
    ...
    rows: (
      ...
      ([数据规模], [最终合并数据集 809 张（train/val/test = 566/162/81），前期累计采集超过千张]),
...
最终的 pose 训练运行采用 Ultralytics YOLOv8s-pose，输入尺寸 960、batch size 为 8、训练 100 轮，启用默认优化器与常规颜色/平移/缩放增强，末 10 轮关闭 mosaic。训练日志显示，模型在第 100 轮时的组件检测指标达到 precision(B)=0.991、recall(B)=0.989、mAP50(B)=0.991、mAP50-95(B)=0.786；对应的引脚 pose 指标达到 precision(P)=0.955、recall(P)=0.954、mAP50(P)=0.947、mAP50-95(P)=0.829。虽然课堂真实拍摄仍会受到遮挡、模糊与反光影响，但这些结果已说明模型能够较稳定地同时完成组件定位与引脚关键点回归，为后续孔位映射与拓扑重构提供可用的视觉前提。
...
真实拍摄常需多角度补拍以缓解遮挡，但多视角同时进入检测又会引入新问题：同一元件可能在不同视图中被重复识别或跨视图错误匹配，使元件编号失稳。为此组件检测采用“主视图优先”策略——以主视图建立全局唯一的元件主实例与编号，侧视图仅作为补充候选而不直接进入主实例列表；先用主视图锚定主事实、再把侧视图当作证据，从而在享受多视角信息的同时抑制误合并。该阶段输入 1 至 3 张面包板图片，输出主视图主实例与侧视图补充候选。

检测输出采用统一结构化格式（含元件编号、类别、封装、置信度与边界框等字段），并过滤面包板本体与布线区域等背景类别；对集成芯片则通过封装识别推断双列直插 8/14 脚，为后续引脚生成提供依据。

== 全图引脚检测

电气诊断要求识别精度下沉到引脚级，而常规目标检测只能给出元件级边界框，无法回答“某引脚究竟插在哪个孔”。一个直觉方案是先裁剪单个元件框、再在框内分别回归引脚，但实践中我们发现裁剪会把引脚切到框外、并丢失元件周边的上下文，导致系统性漏检。为此本系统改采整图关键点检测：在主视图上以 YOLO-Pose 一次性回归全部引脚关键点，再依据类别与边界框的几何约束把引脚归属到对应元件——既避免了裁剪误差与局部上下文丢失，又为后续多视图融合保留了完整证据。

模型不可用时，该阶段输出格式兼容的“不可用”占位结果，保证下游不因模型缺失而结构崩溃；对集成芯片则可不依赖引脚模型，按边界框与面包板行列约束生成 8 或 14 个引脚。每个引脚记录其在各视图的关键点坐标、可见性与综合置信度，为孔位映射提供完整证据。

== 孔位映射与多视图证据融合

把“引脚在图像中接近某处”的模糊观测，上升为“引脚归属某一导通节点”的确定结论，是整条事实链中最易产生歧义的一步：同一关键点可能落在相邻孔位之间，单凭最近邻吸附极易误判。孔位映射因此并不简单选取最近孔，而是先用主视图标定面包板网格（校准失败则回退合成网格并显式标记），再为每个引脚构造融合关键点坐标、可见性、来源与投影的观测记录以生成候选孔位，最后依据板型规则把孔位解析为对应的导通节点。
...
针对遮挡，系统在数据结构层面预留了多视图融合能力：主视图不可见或置信度偏低时提高侧视图权重，避免单一俯视图失效导致整体失准。板型规则在此承担连接视觉与电气逻辑的桥梁，将物理孔位规范化为稳定的导通节点。
...
DSL 以语义化构造接口与运算符重载提升可读性：`input()`/`output()`/`power()`/`ground()` 直接声明带角色的网络，`&` 与下标增量赋值（`comp[pins] += (nets)`）让接线代码贴近原理图直觉；`symmetry(...)` 声明对称网组（如差分对两支路可互换），`compare_options` 携带等价规则（忽略孔位/元件编号、忽略无极性引脚顺序、允许多余导线）。声明后经 `to_logical_reference()` 编译为统一逻辑参考 JSON。如此，新增一个参考电路只需以接近原理图的方式书写数十行 DSL，既降低出错概率，又与比对器保持稳定数据契约。
...
#figure(
  三线表(
    ...
    rows: (
      ...
      ([模板嵌入], [用子图同构搜索 required/optional/forbidden 三层模板], [识别合法变体与拓扑意图]),
...
模板以三层语义刻画结构（如#ref(<tab:tier>)所示）：`required` 缺失则不匹配，`optional` 可缺不扣分，`forbidden` 出现即降级或否决（如反相放大器出现反馈电容，往往说明学生实际搭的是积分器）。模板还携带两类灵活度机制：变体（variants）描述同一拓扑的多种实现——以共射为例内置“直接接地 / 含 R#sub[E] / 含 R#sub[E] 并 C#sub[E]”三变体，匹配时取最高置信度；重数（multiplicity）以 (最小,最大) 约束元件个数，使加法器 2 至 5 路输入电阻均判合法。模板还声明参数化不变量（如差分对 R#sub[C1] ≈ R#sub[C2]）作为面向器件取值的约束框架；因当前视觉管线尚未提取数值，此类不变量按设计仅声明、执行留待后续。
...
在匹配算法上，系统把每份模板（含各变体）编译为小型二分图，以 networkx 子图同构搜索其在学生电路图中的嵌入——用子图同构而非完整同构，是因为学生电路可含模板未提及的多余导线、装饰或重复元件，只要规范结构可被“嵌入”即可。结构得分按已满足的必需边占比计、再按 `forbidden` 命中扣分，给出匹配置信度与角色指派。该模板层与图比对并行、对管线只读，从而把第一章所述“纯规则需不断打补丁”的边界情形（多余元件、合法变体、输入数量变化）系统化地容纳进来。
...
#figure(
  三线表(
    ...
    header: ([维度], [指标], [结果]),
    rows: (
      ([数据资产], [自建引脚级视觉数据集], [自采真实面包板照片 1000 余张，引脚级关键点标注]),
...
综合上述实验，系统在确定性链路、检索契约与边缘部署上的能力已获实测支撑；主要不足在于：其一，教学解释质量的系统量化尚待 gold 评测集与评分模型补齐；其二，蒸馏学生在无接地兜底路径上仍可能生成质量欠佳内容（已由 llm_fallback 标注与校验器约束在事实层面，但语言质量受参数规模所限）；其三，端到端“真实照片→正确诊断”的成功率仍需在更大真板集上统计。
