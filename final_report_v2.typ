...
围绕上述需求，本项目的主要工作如下。（1）提出面向真实拍摄照片的引脚级电气网表重构方法，以关键点检测与孔位吸附替代元件级目标检测，将识别粒度下沉至引脚与孔位；（2）以确定性图比对承担对错判断、智能体仅在受约束证据内生成解释，并由回答合规校验拦截无依据陈述，构成“神经—符号”的可审计诊断链；（3）面向边缘部署，采用双教师蒸馏与 INT4 量化得到端侧教学解释模型，并在 Intel DK-2500 上以 NPU、iGPU、CPU 三个计算单元分工实现本地推理；（4）通过三组消融与端侧实测，分别考察检索约束、模型蒸馏与量化的作用。

= 系统总体设计

== 总体架构与主业务流程

LabGuardian 的核心设计是把“事实生产”与“话术生成”在结构上分开，既用上大模型的语言能力，又不让它参与安全敏感的对错判定。系统不简单叠加视觉模型与大语言模型，而是把诊断拆成逐级递进的三条链路，前两链以确定性算法产出可审计的结构化事实，第三链才由受约束的小模型把事实改写为教学语言，构成神经—符号的可审计诊断，下游的自然语言解释只能以上游可核验的结构化事实为依据。

一次完整诊断的运行流程如#ref(<fig:three-chains>)所示。交互端上传面包板俯视照片并选定参考电路后，服务端编排器依次执行各环节，先用关键点检测与孔位吸附重构出引脚级结构化网表，再以 VF2 确定性图比对把网表与参考电路 DSL 比较、产出带证据引用的结构化错误码，最后由端侧蒸馏小模型在检索约束与合规校验下据错误码生成有依据的教学讲解，并返回交互端可视化。若视觉映射不准确，学生可在前端人工修正孔位、端口或网络角色后调用重算接口，服务端不重跑视觉检测，仅从修正后的元件重新执行拓扑重构、参考验证与解释，以人机协同避免视觉阶段偶发错误导致系统整体不可用。

#figure(
  image("assets/arch_overview.png", width: 100%),
  caption: [LabGuardian 神经—符号诊断的总体架构与端到端运行流程。橙框内是视觉感知与引脚级网表重构两步；其下 VF2 确定性图比对据参考电路 DSL 判定连接、产出可审计的结构化错误码；底部端侧蒸馏小模型在知识库检索与合规校验约束下据错误码生成教学讲解。黑色实线为前向流程，左侧黑色回环表示学生人工修正孔位后触发的重算（不重跑视觉检测）],
) <fig:three-chains>

在架构之上，系统设计遵循四项原则。视觉模型只产生候选事实，最终结论须经板型规则、拓扑分析与验证模块约束；各阶段输出与网表、诊断报告、上下文包均以统一结构化格式为协作边界，避免任何一方私自猜测；允许把 AI 输出修正为可信事实后再重算；保留模型路径、推理参数、版本号与阶段耗时，连同遥测用于分析追溯。
...
#figure(
  image("assets/vf2_compare.png", width: 100%),
...
#figure(
  三线表(
    ...
    header: ([模块], [用例数], [关键断言]),
    rows: (
      ([智能体（意图/上下文/合规校验/路由）], [60+], [意图路由、ReAct 流程、合规校验规则]),
...
诊断智能体统一编排诊断、概念讲解、操作指导与混合问答四类教学意图，共用同一张状态图，仅按意图切换工具白名单、校验规则与回答模板；工具选择进一步取决于错误族（如短路类调用网表追踪与板型查询），智能体据此动态选工具#cite(<react>)，而非把所有知识与历史塞入上下文。意图判定以确定性关键词分类为主，端侧小模型做四类意图分类的准确率与延迟均不及零延迟关键词路由，故路由交给关键词分类、小模型只负责生成（实测消融见本章三组消融实验一节）。

状态图以 LangGraph 为编排外壳，管理节点（错误分类、上下文构建、规划、观察、反思、回答验证、修复、定稿）的流转，并为最小化边缘部署提供行为等价的确定性顺序回退路径。其核心是职责分离，视觉、拓扑、比较与验证模块用于确定电路事实，大模型仅用于将问题改写为自然语言而不介入判定；反思环节设有回答合规校验，由一组固定规则给回答把关，回答必须引用证据、命中相应错误码、危险场景先提醒断电，任一条不满足都判为不合格并触发修复，小模型无法跳过这一步。即便解释模型仅十亿级参数，一旦偏离既定事实即被结构化拦截，无需依赖提示工程。这正是端侧小模型在正确性敏感场景可靠应用的前提。
...
蒸馏数据不是让教师模型自由发挥生成的，而是严格建立在系统已经固定好的结构化证据上。项目选用本地部署的 Qwen3-32B#cite(<qwen3>)和云端的 DeepSeek-V3#cite(<deepseekv3>)作为双教师，让它们在同一份固定证据上分别作答，把回答内容和可核查的事实对应起来，尽量减少幻觉。候选问题围绕典型电路、常见故障和学生常问的问题自动扩展，再结合人工与规则过滤掉不贴近课堂场景的样本；教师回答也不会直接拿来训练，只保留有明确依据、没有被降级为“证据不足”、长度合适且表达基本一致的样本。这里的“证据不足”不是主观判断，而是看教师回答能否给出当前证据里的合法引用；如果引用无法在场景、错误码、故障案例或工具结果中核对，回答就会被自动降级为“证据不足”。最终从 4990 条候选教师输出中保留 3466 条高质量样本，并构建为 3450 条正式 SFT 数据（候选留存率约 69.1%）。学生模型以 Qwen2.5-1.5B-Instruct#cite(<qwen25>)为基座，经 LoRA#cite(<lora>)微调并合并权重后，再进行 INT4 量化#cite(<awq>)并导出为 OpenVINO 部署模型，合并后约 3.1 GB、INT4 约 0.88 GB，拷贝到板端即可运行，全程无需外网。

从训练架构上看，这里采用的是比较直接的 teacher-student 方案。双教师模型负责在同一份固定证据上生成高质量答案，学生模型则以 Qwen2.5-1.5B-Instruct 为基座，学习如何根据这些证据给出课堂讲解。也就是说，教师模型提供的是可学习的目标回答，学生模型学习的是“问题 + 固定证据 -> 教学解释”这一映射，而不是单独去模仿大模型的自由表达。

训练目标方面，本项目没有采用传统知识蒸馏里常见的 logits 对齐或 KL 散度损失，而是把双教师生成并筛选后的答案整理成监督微调数据，对学生模型做 LoRA 监督微调。这样做的好处是实现简单，和部署形态一致，也更符合本项目“基于证据回答”的目标。换句话说，这里的蒸馏更接近基于教师答案的数据蒸馏，而不是额外设计多项蒸馏损失的经典 KD。

训练优化方式也尽量保持工程可落地。学生模型训练使用 AdamW 优化器，学习率设为 5e-5，并采用 cosine 学习率调度、warmup、weight decay 和梯度裁剪；训练过程使用 bf16，并通过梯度累积控制显存占用。训练完成后得到的是 LoRA 权重，再与基座模型做权重合并，最后导出为 INT4 OpenVINO 部署模型。上述流程既说明了蒸馏训练的整体架构，也交代了学生模型从微调、权重合并到量化部署的完整路径。
...
和常见那种强调“把大模型能力搬给小模型”的思路不同，本项目更关注让小模型在课堂场景里答得稳、答得准、答得安全。教师答案必须基于固定证据，尽量减少幻觉；训练时和板端部署时用的是同一套检索规则和场景白名单，避免训练一套、部署另一套；学生模型后面还有回答合规校验兜底，只要回答越过结构化事实边界，就会被拦下来。因此，这里更像是在训练一个“会看证据说话”的端侧教学解释模型。
...
这部分主要是对蒸馏后的学生模型做一个简要结果检查，而不是把篇幅放在细碎指标上。为减少主观判断，我们引入了自动评分。具体做法是让 DeepSeek-V3 作为独立评委，在 `temperature=0` 的设置下，对每道回答从正确性、教学性、简洁性、格式结构和依据安全性五个维度分别打 1 到 5 分。作为对照，我们还用了合规校验规则再打一次分，比如是否先提醒安全、有没有提到关键故障点、步骤是否完整。两套自动评分的整体趋势一致，因此这里保留一组最能说明问题的四模型对比结果。
...
表#ref(<tab:key-results>)先汇总关键量化结果，主要证据不集中于单一模型精度，而是覆盖数据资产、确定性算法、检索约束、模型压缩、边缘部署与工程测试六个维度，对应“把真实感知、符号判断、智能解释与边缘部署放在同一套可验证闭环中”的设计取向。
...
本文提出并实现了 LabGuardian，一个面向电子实验教学、运行于英特尔边缘平台的神经—符号电路诊断系统。系统以真实拍摄的面包板照片为起点，经关键点检测与孔位吸附重构引脚级电气网表，由确定性图比对承担可审计的对错判断，再由受约束的智能体在回答合规校验约束下生成有依据的解释。教学解释模型经双教师蒸馏与 INT4 量化部署于 DK-2500，以 NPU、iGPU、CPU 三个计算单元分工实现本地推理。板端实测表明，系统可在 8 GB 内存预算内完成视觉识别与端侧解释，延迟、吞吐与功耗满足课堂实时使用；检索约束使同一小模型从无法指认故障转为正确指出故障，而教学解释的事实正确性由回答合规校验保证。

与既有工作相比，本系统主要有三点不同。其一，面向必然带有干扰的真实照片输入，而非理想的干净网表；其二，以神经—符号的思路把对错判断交给可审计的符号系统、把语言表达交给受约束的小模型；其三，让蒸馏训练与板端部署使用同一套检索约束，再配合回答合规校验，把防幻觉做成工程上可审计的约束而非提示技巧。这样一个十亿级小模型也能在对正确性要求较高的教学场景中、于边缘设备上稳定运行。
