...
#let stack-of-bars = stack(
  spacing: 7pt,
  ..layers.map(((name, mods, emph)) => layerbar(name, mods, emph)),
)

#box(width: 15.2cm)[
  #grid(
    columns: (0.9cm, 1fr, 0.9cm),
    column-gutter: 6pt,
    align: horizon,
    grid(
      columns: (auto, auto),
      ...
      box(height: 7.2cm, width: 8pt)[
        #place(center + horizon, line(
          start: (0pt, 0pt), end: (0pt, 100%),
          stroke: 0.9pt + black,
        ))
        #place(bottom + center, polygon(
          fill: black,
          (0pt, 0pt), (4pt, -8pt), (-4pt, -8pt),
        ))
      ],
    ),
    stack-of-bars,
...
为消除人工评分的主观性与 3 点二元量尺的粒度过粗问题，本节引入 LLM-as-Judge 自动评分体系：以 DeepSeek-V3 为独立评委（temperature=0，确保可复现），对每题回答按正确性、教学性、简洁性、格式结构与接地安全性五个维度分别给出 1—5 分。五维 1-5 量尺的粒度（25 个有意义的区分档位）远优于原 3 点二元命中制，且所有评分均可复现、可审计。作为对照，同时以确定性校验器规则（安全前置、故障关键词、预期要点命中、结构化步骤等）进行独立评分，两轨自动化评分与人工评分趋势一致但提供更细粒度的诊断信息。

=== 部署版 INT4 的 P0 质量剖面

部署版 INT4 学生模型在 30 题固定集上的 LLM-Judge 评分结果如#ref(<fig:p0-judge-quality>)与#ref(<tab:judge-quality>)所示：五维综合得分 3.37/5，其中正确性最高（3.90/5）、简洁性与格式结构良好（4.17/5 与 3.60/5），教学启发性（2.57/5）与接地安全性（2.87/5）为相对短板。按意图细分，实验指导（3.60/5）与概念讲解（3.38/5）表现最好，诊断排查（3.23/5）最具挑战——这与校验器评分及人工评分揭示的趋势一致，但五维量尺更精确地定位了短板维度：教学启发性的偏低与当前短答格式限制（每题约 120 字、3 行约束）直接相关，并非模型在长文本场景下的固有能力不足。

#figure(
  image("board_data/p0_int4_quality_judge.png", width: 94%),
  caption: [部署版 INT4 学生模型的 LLM-Judge 五维质量剖面：左为正确性、教学性、简洁性、格式结构、接地安全五维得分（1-5 分量表）；右为按教学意图细分的综合得分——实验指导与概念讲解最好，诊断排查最具挑战],
...
`P0.5` 以同一题集、同一约束、同一 LLM 评委对比本地 `FP` merged 与部署版 `INT4`，结果如#ref(<fig:p05-judge-quality>)与#ref(<tab:p05-judge-quality>)所示。LLM-Judge 综合得分：`INT4` 3.37/5、`FP` 3.33/5（Δ=+0.04），正确性上 `INT4` 3.90/5、`FP` 3.87/5，通过率上 `INT4` 90.0%、`FP` 83.3%。按意图细分，`INT4` 在实验指导（+0.20）与诊断排查（+0.15）上相对 `FP` 有稳定优势，`FP` 在混合问答上略高（+0.25）。量化后未见质量退化，在综合得分、通过率与多数意图上 `INT4` 部署版反而略优——这消除了原人工 3 点评分中 `FP`（64.4%）低于基座（66.7%）的反直觉结果，验证了量化流程的正确性。
