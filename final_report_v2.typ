...
数据集采集自真实实验台照片，覆盖一阶 RC、共射放大、差分对、UA741 反相/积分/加法六类教学拓扑，并刻意纳入多角度、不同光照与导线遮挡等真实变体，样本本身即贴近课堂实拍的情景、无需合成增强。标注采用“组件级边界框 + 引脚级关键点”双层结构（关键点张量 3×3，每实例至多 3 引脚，记录横纵坐标与可见性），把识别精度从元件级下沉到引脚级。这正是常规检测数据集所不具备、却为电气诊断所必需的粒度。数据集标注电阻、陶瓷电容、电解电容、二极管、发光二极管、跳线、三脚晶体管共七类教学元件，并按训练 / 验证 / 测试留出划分以支持独立评测。
...
确定性主张的工程基础是各模块在统一约定下行为可复现、可回归。后端以 pytest 组织约 900 余个单元与约定用例，覆盖智能体（意图路由、ReAct 流程、合规校验）、检索与知识库、确定性图比对、视觉管线等模块，整体通过率约 99%（个别失败为预先存在的环境依赖）。
