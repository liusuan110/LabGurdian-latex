...
#figure(
  三线表(
    ...
    rows: (
      ...
      ([concept_tutor], [8], [3.38], [3.88], [2.63], [4.13]),
      ([mixed], [4], [3.50], [4.00], [2.50], [4.25]),
      ([lab_guidance], [5], [3.60], [4.00], [2.80], [4.20]),
      ([diagnostic], [13], [3.23], [3.85], [2.46], [4.15]),
    ),
  ),
...
#figure(
  ...
  caption: [P0 四模型 LLM-Judge 评分：上图为综合得分与正确性（1-5分量表，分组柱形），下图为各模型 ≥3 分通过率。四模型呈清晰单调排序（教师 > INT4 > FP > 基座），蒸馏与量化未造成质量退化],
) <fig:p0-four-judge>
...
系统在确定性链路、检索契约与边缘部署上的能力已获实测支撑；主要不足在于：评测题量仅 30 题、评分者模型与答题者部分同源（虽以 temperature=0 与确定性校验器交叉验证降低随机偏差）；蒸馏学生在无接地兜底路径上的语言质量受参数规模所限（事实层面已由校验器约束）；端到端"真实照片→正确诊断"的成功率仍需在更大真板集上统计；LLM-as-Judge 自动评分虽大幅改善了可复现性与粒度，其与真实教学场景中教师评价的生态效度仍有待进一步校准。
