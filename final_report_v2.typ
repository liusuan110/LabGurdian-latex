...
这条事实链落到工程上有几条不能破的约束。视觉模型只产生候选，最终结论必须经板型规则、拓扑分析与验证模块确认；各环节之间只认统一的结构化格式（网表、诊断报告、上下文包），谁都不能私自猜测；视觉难免出错，允许学生把个别引脚或端口改对后重算，不必从头再来；每次运行都留下模型路径、推理参数、版本号与各阶段耗时，连同硬件遥测存档，便于复现与追溯。
