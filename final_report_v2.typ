...
从质量侧看，未蒸馏基座的平均得分率为 66.7%，本地 `FP` merged 为 64.4%，部署版 `INT4` 为 71.1%，教师上限达到 92.2%。这说明在当前固定题集与实现链路下，蒸馏学生的 `FP` 版本并未稳定优于基座，提示“蒸馏后能力迁移”仍受题集覆盖与生成链路细节影响；而部署版 `INT4` 反而较基座提高 4.4 个百分点、较本地 `FP` 提高 6.7 个百分点，说明当前量化部署并未带来可观测的质量退化。更重要的是，教师上限仍比 `INT4` 高出 21.1 个百分点，表明当前学生模型与高质量教师回答之间仍有显著改进空间。

从时延侧看，本地基座约 4.26 秒 / 题、`FP` 约 5.80 秒 / 题、部署版 `INT4` 约 7.73 秒 / 题，而教师 API 约 1.32 秒 / 题。三条本地链路现已处于同一量级，说明该列对推理后端与生成收敛状态较为敏感。需要注意，这里的时延受推理后端与部署环境共同影响，教师列走的是云端 API、学生列分别走本地 HuggingFace 与 OpenVINO 链路，因此该列主要用于反映“当前系统实现下的端到端响应成本”，而不应被解释为不同模型参数规模之间的纯推理速度对决。就当前工程目标而言，四模型矩阵已经明确回答了三个问题：`INT4` 部署版具备可用教学解释质量；它相对本地 `FP` 未出现明显质量退化；而教师上限与当前学生模型之间仍存在值得继续追赶的质量缺口。

#figure(
  image("board_data/p0_four_model_matrix.png", width: 94%),
  caption: [P0 固定题集四模型横向矩阵：左图比较未蒸馏基座、蒸馏学生 `FP`、蒸馏学生 `INT4` 与教师上限在平均得分率、通过率和满分率上的表现，右图给出当前实现链路下的单题平均生成时延。结果表明，部署版 `INT4` 在当前题集上优于基座与本地 `FP`，而教师上限仍保留明显质量优势。],
) <fig:p0-four-model>
