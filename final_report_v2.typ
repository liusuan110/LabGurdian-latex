...
高校电子类基础实验是学生由理论走向工程实践的关键环节：学生需依据原理图，在面包板上插接电阻、电容、二极管、发光二极管、晶体管和运算放大器等元件，并借助电源、示波器和函数信号发生器等工具来对电路进行实验。这一过程要求学生完成三重映射——由电路原理到元件功能、由二维原理图到面包板孔位、由实验现象到故障原因，任一映射失败都可能使实验停滞。

电子实验课普遍师生比例不匹配，教师难以同时照顾到所有学生。学生向教师提出问题后，教师还需逐项排查元件是否插对、引脚是否接反、孔位是否正确、电源是否误接等基础问题。此类排查高度依赖经验、重复性强，却不能省略——极性反接与电源短路可能损坏芯片，甚至带来安全风险。

上述场景对智能诊断提出了四个超出常规视觉识别的核心难点。其一，真实图像遮挡严重：导线交叠、元件遮挡引脚与拍摄角度差异都会干扰识别，而常规目标检测只给元件级边界框、无法判定引脚插入哪一孔位，因而诊断必须深入引脚级与孔位级。其二，面包板导通关系不直观：主区按列导通、中缝隔离左右半区、电源轨可能分段，须将物理孔位映射为导通节点，才能判定相邻孔位并非同网、或两引脚短接于同一网络。其三，教学需要可解释、可在界面高亮的证据链而非黑盒结论（具体错误码与证据字段见第四章）。其四，系统须适应边缘环境：实验室网络未必稳定、数据不宜全部上云，故以边缘设备为主要运行平台、降低对外网依赖。
...
因此，本文采取折中且工程可落地的路线：由确定性算法构建可验证的结构化事实链承担事实判断，再由智能体在受约束的证据范围内生成解释，从而在可解释性与安全性之间取得平衡。这一“神经—符号解耦”的设计取向贯穿全文，也是本系统区别于上述三类方案的根本所在。
...
围绕上述需求，本项目的主要工作与创新点如下：（1）提出面向噪声感知输入的引脚级电气网表重构方法，以关键点检测与孔位吸附替代元件级目标检测，将识别粒度下沉至引脚与孔位；（2）以确定性图比对承担对错判断、智能体仅在受约束证据内生成解释，并以不可绕过的确定性校验器拦截无依据陈述，构成“神经—符号解耦”的可审计诊断链；（3）面向边缘部署，采用双教师蒸馏与 INT4 量化得到端侧教学解释模型，并在 Intel DK-2500 上以 NPU、iGPU、CPU 异构分工实现低功耗本地推理；（4）通过三组正交消融与端侧实测，分别验证检索契约、模型蒸馏与量化各自的贡献。
...
#diagram(
  ...
  // ── 行名(左侧小字) ──
  node((0, 0), text(weight: "bold")[感知\ 事实链], stroke: none, inset: 2pt),
  node((0, 2), text(weight: "bold")[符号\ 判定链], stroke: none, inset: 2pt),
  node((0, 4), text(weight: "bold")[接地\ 解释链], stroke: none, inset: 2pt),
  ...
  node((4.7, 4), text(fill: white)[接地\ 教学解释], fill: luma(62), name: <c3>),
  edge(<c1>, <c2>, "-|>", stroke: 0.6pt + black),
  edge(<c2>, <c3>, "-|>", stroke: 0.6pt + black),

  // ── 链间纵向传递(竖直向下箭头) ──
  edge(<a3.south>, (5.2, 2), "-|>", stroke: 0.6pt + black,
       label: text(8pt, fill: luma(90))[引脚级网表], label-side: right, label-sep: 3pt),
  ...
  node((5.6, 2.78), text(8pt, fill: luma(60))[事实生产（可证伪 · 可审计）], stroke: none, inset: 1pt),
  node((5.6, 3.22), text(8pt, fill: luma(60))[话术生成（受约束）], stroke: none, inset: 1pt),

  // ── 校验失败回环(向下弯曲虚线,行下方) ──
  edge(<c2.south>, <c1.south>, "-|>", bend: 42deg,
...
#context {
  ...
  let layers = (
    ...
    ("Agent 与知识解释层", "上下文编排 · 工具调用 · 检索契约 · 校验器（唯一接触大模型）", true),
    ("基础支撑层",         "异步任务 · 缓存 · 容器化 · 单元/契约测试",                false),
  )
  let layerbar(name, mods, emph) = box(
    width: BARW,
    stroke: (if emph { 1.3pt } else { 0.7pt }) + black,
    radius: 3pt, inset: 0pt,
  )[#grid(
      ...
      rect(width: 0.7pt, height: 100%, fill: luma(120), stroke: none),
      text[#mods],
  )]
  ...
  let varrow(up) = box(width: 8pt, height: H)[
    ...
    #if up { place(top + center, polygon(fill: black, (0pt, 7pt), (8pt, 7pt), (4pt, 0pt))) } else { place(bottom + center, polygon(fill: black, (0pt, 0pt), (8pt, 0pt), (4pt, 7pt))) }
  ]
  grid(
    ...
    bars,
    varrow(true),
    rotate(90deg, reflow: true, text(style: "italic")[逐层向上提供可证伪事实]),
  )
}
  ],
  caption: [系统六层总体架构：两侧箭头分别表示自上而下的调用与自下而上的可证伪事实流；加粗描边的两层（流水线事实层、Agent 与知识解释层）即“事实生产”与“话术生成”的结构边界],
) <fig:arch>
...
#diagram(
  ...
  node-inset: 7pt,
  node-corner-radius: 3pt,
  ..main.enumerate().map(((i, s)) => node((i, 0), s,
      stroke: (if s in emphs { 1.3pt } else { 0.7pt }) + black)),
  ...
  edge((4.3, 1.05), (3, 0), "-|>", bend: 32deg, stroke: dash,
       label: text(7.5pt, fill: rgb("#555"))[不重跑视觉，从修正元件起重算],
       label-pos: 0.55, label-side: right),
)
  ],
...
在技术选型上，服务端以 Python 与 FastAPI 构建异步接口，视觉识别采用 YOLO 系列模型经 OpenVINO 适配英特尔异构硬件。电气比对基于图论库实现确定性图同构与子图匹配，知识检索使用本地多通道知识库。端侧解释模型由开源中文基座（Qwen2.5-1.5B，详见第六章）蒸馏量化得到，交互端以画布叠加方式呈现识别与诊断结果。
...
本系统将“看图诊断电路”分解为组件检测、引脚检测、孔位映射、拓扑重构和参考比较几个明确的步骤。这几个步骤分别负责一段流程，而智能体仅负责解释，使每一步可独立测试、可视化与修正，从根本上抑制端到端模型难以审计、易于幻觉的风险。
...
最终的 pose 训练运行采用 Ultralytics YOLOv8s-pose#引[7]，输入尺寸 960、batch size 为 8、训练 100 轮，启用默认优化器与常规颜色/平移/缩放增强，末 10 轮关闭 mosaic。训练日志显示，模型在第 100 轮的验证集上，组件检测框（B）指标为 precision=0.991、recall=0.989、mAP50=0.991、mAP50-95=0.786，引脚关键点（P）指标为 precision=0.955、recall=0.954、mAP50=0.947、mAP50-95=0.829。上述验证集指标说明模型能较稳定地同时完成组件定位与引脚关键点回归，为孔位映射与拓扑重构提供可用的视觉前提；测试集未参与任何训练与模型选择，留作端到端系统级评测的独立样本。
...
这一步的歧义并非工程实现不足，而是密集小目标检测的固有难点。面包板上的引脚是典型的密集小目标：数十至上百个孔位以毫米级间距规则排布，单个引脚在俯视图中仅占十余像素，相邻孔位外观近乎一致，且常被导线与元件本体部分遮挡。密集小目标的关键点定位是计算机视觉中最具挑战的场景之一——即便换用更强的检测与关键点模型，在“引脚究竟落在相邻哪一孔”上也难以做到 100% 正确，孔位级的残余误差不可完全消除。因此系统并不以追求一个不可达的完美视觉为目标，而是正视这一残差、将其显式暴露并交由低成本的人在回路消解。
...
#figure(
  ...
  caption: [低置信引脚的歧义可视化：不强行吸附，以虚线环标出候选孔位、交由学生一键修正，把“网匹配歧义”显式化为人机协同介入点],
) <fig:ambiguity>
...
当完整同构失败时，算法逐级降级。先判断学生电路是否为参考电路的子图或包含关系，仍不成立时，才以图编辑距离或近似相似度给出错接报告。匹配规则采用“严格/宽松”分级——电源与地、以及功能引脚严格匹配；电阻、电容、导线等无极性两脚器件忽略引脚顺序；并允许合理的额外导线。相比逐孔位规则，该分级显著降低了因摆放差异导致的误报；相比纯文本规则，又对关键电气语义保持刚性约束——这正是把“对错判断”交给符号系统、而非交给易幻觉大模型的根本理由。
...
#diagram(
  ...
  edge(<d1>, <p1>, "-|>", stroke: 0.6pt + black, label: text(8pt, fill: luma(90))[否], label-pos: 0.5, label-side: left),
  edge(<p1>, <d2>, "-|>", stroke: 0.6pt + black),
  edge(<d2>, <p2>, "-|>", stroke: 0.6pt + black, label: text(8pt, fill: luma(90))[否], label-pos: 0.5, label-side: left),
...
纯图比对需要一份确定的参考电路，但同一教学拓扑常有多种正确的实现方式，以单一参考硬比较极易误报。为此系统引入拓扑模板，把每类规范拓扑表达为“偏规范”，以三层语义刻画结构（如#ref(<tab:tier>)所示）：`required` 缺失则不匹配，`optional` 可缺不扣分，`forbidden` 出现即降级或否决；另以变体与重数容纳多种合法实现，并声明参数化不变量。
...
诊断报告还包含面向交互端的高亮协议，把每条诊断项的证据引用转换为可直接绘制的高亮目标，智能体生成解释时复用同一份协议，使自然语言说明与界面定位指向同一份事实。本系统从俯视照片到引脚级结构化网表的端到端贯通示例见#ref(<fig:e2e>)。
...
诊断智能体统一编排诊断、概念讲解、操作指导与混合问答四类教学意图：共用同一张状态图，仅按意图切换工具白名单、校验规则与回答模板（如#ref(<fig:intent>)所示）；工具选择进一步取决于错误族（如短路类调用网表追踪与板型查询），智能体据此动态选工具#引[10]，而非把所有知识与历史塞入上下文。意图判定以确定性关键词分类为主：端侧小模型做四类意图分类的准确率与延迟均不及零延迟关键词路由，故路由交给关键词分类、小模型只负责生成（实测消融见第七章）。
...
#diagram(
  ...
  node((3, 1.5), [统一 \ 状态图], stroke: 1.3pt + black, corner-radius: 5pt, shape: fletcher.shapes.rect, inset: 12pt, width: 28mm),
  ...
  // fan-out edges
  edge((3, 1.5), (6, 0.5), "-|>", stroke: 0.6pt + black, label: text(8pt, fill: luma(90))[按意图切换], label-side: left, label-pos: 0.32),
...
上述约定构成“训练即部署”的检索不变量：蒸馏数据生成与板端部署使用同一套代码、知识与场景白名单，并由起飞前自检脚本校验两端等价。它把“可用知识的边界”做成可审计的工程契约——场景白名单按拓扑过滤器件手册，使反相放大器的问题不会召回无关的定时器芯片；故障案例仅在错误码命中时返回，召回键正是第四章图比对输出的错误码词表。检索缺失时系统 fail-closed：蒸馏样本拒绝生成、部署端回退确定性模板，宁可少答也不让错误默认值污染结论。这一取向与近年电子设计自动化语言模型“以确定性结构约束生成”的方向一致#引[3]。
...
作为本章诊断闭环的整体印证，#ref(<fig:diag-demo>)给出一例反相放大器双故障的端到端诊断：系统在真实故障板上同时定位“输入电阻错接至偏置脚”与“运放负电源缺失”两处接线错误，由确定性比对输出稳定错误码、再由端侧学生模型生成接地讲解，使自然语言解释与界面高亮指向同一份结构化事实——这正是“噪声感知输入 → 确定性诊断 → 受约束解释”闭环在真实故障样本上的完整呈现。

#figure(
  image("pictures/cadx/diag_demo.pdf", width: 100%),
  caption: [反相放大器双故障端到端诊断示例（板上实拍 + 真实管线输出）：(a) 故障板的组件/引脚检测与错误定位——①输入电阻错接至偏置/调零脚 pin1（应接反相输入 pin2），②运放 pin4（V−）未接负电源 VEE；(b) 确定性比对输出的结构化错误码（`WRONG_CONNECTION`、`OPEN_CIRCUIT`）与端侧学生模型生成的接地讲解，自然语言解释与错误定位指向同一份事实],
) <fig:diag-demo>
...
本章关注“在哪里运行、跑得如何”，先给出部署方式，再说明 CPU、iGPU 与 NPU 的异构分工并以板端实测的延迟、吞吐与功耗验证，最后介绍端侧小模型蒸馏与初步评测。
...
为量化上述分工，项目以 turbostat 按 0.25 秒间隔采样 RAPL 能量计，并在板端以输入分辨率 960 单进程串行采集 60 至 1153 次推理统计，测得 YOLOv8s-pose 关键点检测在 CPU、iGPU、NPU 三种单元上的延迟、吞吐与功耗。其功耗时序如#ref(<fig:power-ts>)所示：NPU 推理期间 CPU 核心与 iGPU 功率曲线全程贴近空闲基线（约 4.4 W），说明 NPU 几乎不占用其他单元资源，为“NPU 负责视觉、iGPU 负责诊断、CPU 负责控制”的三路异构分工提供了硬件层面的可行性。
...
各配置详细指标如#ref(<tab:hetero>)所示：NPU INT8 在所有维度均最优（延迟 13.37 ms、吞吐 74.7 img/s、P99 仅 15.61 ms），满足课堂实时视频流需求；INT8 经 144 张校准图训练后量化得到，模型由 22 MB 压至 11 MB，NPU 吞吐由 FP16 的 60.4 升至 INT8 的 74.7 img/s。
...
#figure(
  三线表(
    ...
    rows: (
      (
        [证据冻结],
        [收集 scene、error codes、fault cases、datasheet hits 与 pipeline snapshot],
        [缺场景或非法场景拒绝生成],
      ),
      ([双教师生成], [Qwen3-32B 与 DeepSeek-V3 在同一证据上作答，并统一进入筛样流程], [教师只能引用已给证据]),
      (
        [学生微调],
        [Qwen2.5-1.5B-Instruct + LoRA 监督微调并合并权重；正式 SFT 数据约 3450 条],
        [训练样本与部署检索契约一致],
      ),
...
#figure(
  三线表(
    ...
    rows: (
      ([DeepSeek-V3], [99.8%], [69.5%], [67.8%], [2.56 s], [稳定性最高，作为稳定教师分支]),
  ...
  三线表(
    ...
    rows: (
      ...
      (
        [高频错误标签],
        [缺件/悬空/错接为主],
        [MISSING_COMPONENT 27.5%，FLOATING_CONNECTION 27.1%，WRONG_NODE_CONNECTION 19.7%],
      ),
...
作为对照，同一模型改在板端 CPU 运行虽吞吐相近，但整封装平均功率升至约 43 W、约为 iGPU 的 4.7 倍。这印证了设备归属决策：把自回归的解释生成放在 iGPU 以更低功率维持吞吐，把 CPU 留给拓扑与检索等控制面，而将大模型蒸馏到 1.5B 正是其装入边缘内存与功耗预算的前提。
...
本章围绕三项核心设计主张组织实验证据：对错判断由确定性符号链路给出、可审计无幻觉；检索契约使端侧小模型不换模型即“答得准、答得安全”；系统可在英特尔异构边缘平台低功耗部署。功能与质量评测在开发机上以确定性方式运行，板端性能与能耗在 DK-2500 上以 turbostat 采样 RAPL 实测。
...
实测对照表明，端侧小模型直接做四类意图分类，在规范与口语化混合问题上的准确率不及关键词路由，且每次引入约 1.5—2.5 秒额外时延。系统据此采用零延迟的确定性关键词路由承担意图判定，小模型只负责生成——以实测消融为据，而非经验默认。

=== 蒸馏与量化消融（教学解释质量评测）

为消除人工评分的主观性与 3 点二元量尺的粒度过粗问题，本节引入 LLM-as-Judge 自动评分体系：以 DeepSeek-V3 为独立评委（temperature=0，确保可复现），对每题回答按正确性、教学性、简洁性、格式结构与接地安全性五个维度分别给出 1—5 分。五维 1-5 量尺的粒度（25 个有意义的区分档位）远优于原 3 点二元命中制，且所有评分均可复现、可审计。作为对照，同时以确定性校验器规则（安全前置、故障关键词、预期要点命中、结构化步骤等）进行独立评分，两轨自动化评分趋势一致，但提供更细粒度的诊断信息。

=== 部署版 INT4 的 P0 质量剖面

部署版 INT4 学生模型在 30 题固定集上的 LLM-Judge 评分结果如#ref(<fig:p0-judge-quality>)与#ref(<tab:judge-quality>)所示：五维综合得分 3.37/5，其中正确性最高（3.90/5）、简洁性与格式结构良好（4.17/5 与 3.60/5），教学启发性（2.57/5）与接地安全性（2.87/5）为相对短板。按意图细分，实验指导（3.60/5）综合最高、混合问答（3.50/5）次之，诊断排查（3.23/5）最具挑战——这与校验器规则评分揭示的趋势一致，但五维量尺更精确地定位了短板维度：教学启发性的偏低与当前短答格式限制（每题约 120 字、3 行约束）直接相关，并非模型在长文本场景下的固有能力不足。

#figure(
  image("board_data/p0_int4_quality_judge.png", width: 94%),
  caption: [部署版 INT4 学生模型的 LLM-Judge 五维质量剖面：左为正确性、教学性、简洁性、格式结构、接地安全五维得分（1-5 分量表）；右为按教学意图细分的综合得分——实验指导最高、混合问答次之，诊断排查最具挑战],
) <fig:p0-judge-quality>

#figure(
  三线表(
    columns: (1.7fr, 0.8fr, 1fr, 1fr, 1fr, 1fr),
    align-cells: (left, center, center, center, center, center),
    header: ([P0 分组], [题数], [综合], [正确性], [教学性], [简洁性]),
    rows: (
      ([整体（INT4 部署版）], [30], [3.37], [3.90], [2.57], [4.17]),
      ([concept_tutor], [8], [3.38], [4.00], [2.38], [4.38]),
      ([mixed], [4], [3.50], [3.75], [2.75], [4.00]),
      ([lab_guidance], [5], [3.60], [4.40], [2.80], [4.40]),
      ([diagnostic], [13], [3.23], [3.69], [2.54], [4.00]),
    ),
  ),
  caption: [部署版 INT4 LLM-Judge 五维评分摘要（1-5 分量表）；教学启发性为全系统短板，诊断排查综合得分最低],
) <tab:judge-quality>

=== 量化前后消融：FP vs INT4

`P0.5` 以同一题集、同一约束、同一 LLM 评委对比本地 `FP` merged 与部署版 `INT4`，结果如#ref(<fig:p05-judge-quality>)与#ref(<tab:p05-judge-quality>)所示。LLM-Judge 综合得分：`INT4` 3.37/5、`FP` 3.33/5（Δ=+0.04），正确性上 `INT4` 3.90/5、`FP` 3.87/5，通过率上 `INT4` 90.0%、`FP` 83.3%。按意图细分，`INT4` 在实验指导（+0.20）与诊断排查（+0.15）上相对 `FP` 有稳定优势，`FP` 仅在混合问答上略高。量化后未见质量退化，在综合得分、通过率与多数意图上 `INT4` 部署版反而略优；在更细粒度的 LLM-Judge 五维量尺下，蒸馏 `INT4` 与 `FP` 综合得分均不低于未蒸馏基座（3.37、3.33 对 3.23，详见#ref(<tab:p0-four-judge>)），验证了蒸馏与量化流程的正确性。

#figure(
  image("board_data/p0_5_fp_vs_int4_judge.png", width: 94%),
  caption: [P0.5 FP vs INT4 LLM-Judge 对比：左为综合得分、正确性与通过率对比，右为按教学意图细分。`INT4` 综合略优、通过率高出 6.7 个百分点，量化未造成质量退化],
...
#figure(
  三线表(
    ...
    align-cells: (left, center, center, center, center, center),
    header: ([P0.5 对比项], [综合 /5], [正确性 /5], [通过率], [诊断排查], [实验指导]),
    rows: (
      ([本地 FP merged], [3.33], [3.87], [83.3%], [3.08], [3.40]),
      ([部署版 INT4], [3.37], [3.90], [90.0%], [3.23], [3.60]),
      ([INT4 - FP], [+0.04], [+0.03], [+6.7 pp], [+0.15], [+0.20]),
    ),
  ),
  caption: [P0.5 量化前后 LLM-Judge 评分对比摘要（`FP` 为本地 HuggingFace merged 链路、`INT4` 为 OpenVINO 部署链路）],
) <tab:p05-judge-quality>

=== 四模型横向矩阵与维度剖面对比

项目进一步以 LLM-Judge 对未蒸馏基座、蒸馏 `FP`、蒸馏 `INT4` 与教师上限四模型在同一 30 题集上评分，结果如#ref(<fig:p0-four-judge>)与#ref(<tab:p0-four-judge>)所示。四模型综合得分呈清晰单调排序：教师（3.93）> `INT4`（3.37）> `FP`（3.33）> 基座（3.23），蒸馏与量化未造成退化。各模型五维剖面如#ref(<fig:p0-dim-panel>)所示，教学启发性是全系统共性短板（最高仅 2.93/5），与短答格式约束直接相关；接地安全性上蒸馏 `FP` 最高（3.57/5），体现了检索契约训练对事实锚定的改善。

相比原有 3 点人工 rubric，LLM-Judge 自动评分按维度与意图的细粒度分解直接指向优化方向——诊断排查类题目与教学启发性维度是后续蒸馏数据增强与提示约束优化的重点。

#figure(
  image("board_data/p0_four_model_judge.png", width: 94%),
  caption: [P0 四模型 LLM-Judge 评分：上图为综合得分与正确性（1-5分量表，分组柱形），下图为各模型 ≥3 分通过率。综合得分呈清晰单调排序（教师 > INT4 > FP > 基座），蒸馏与量化未造成质量退化],
) <fig:p0-four-judge>
...
#figure(
  三线表(
    ...
    align-cells: (left, center, center, center, center, center),
    header: ([四模型], [综合 /5], [正确性], [教学性], [简洁性], [接地性]),
    rows: (
      ([未蒸馏基座], [3.23], [3.77], [2.40], [4.27], [2.63]),
      ([蒸馏学生 FP], [3.33], [3.87], [2.60], [4.07], [3.57]),
      ([蒸馏学生 INT4], [3.37], [3.90], [2.57], [4.17], [2.87]),
      ([教师上限], [3.93], [4.73], [2.93], [4.90], [3.43]),
    ),
  ),
  caption: [P0 四模型 LLM-Judge 五维评分摘要；教学性为全系统共性短板，接地性上蒸馏模型相对于基座有稳定改善],
) <tab:p0-four-judge>

#figure(
  image("board_data/p0_dimension_panel.png", width: 94%),
  caption: [四模型五维评分剖面对比（LLM-Judge，1-5 分量表）：各模型正确性与简洁性稳定较高，教学启发性偏低为系统级特征；蒸馏 FP 的接地安全性改善最为显著],
) <fig:p0-dim-panel>

进一步以一个反相放大器相关的具体问题作定性抽样：对于"我把运放的两个输入接反了，会有什么现象？"这一训练分布内样例，蒸馏后的 `INT4` 学生模型能够较快聚焦到"输出饱和到电源轨附近"这一课堂实验中最关键的异常表现，并把回答收束在故障机理与直接现象上；未蒸馏基座模型虽然同样判断"电路无法正常工作"，但后续表述更偏一般性说明，范围较宽，对实验语境下核心症状的聚焦度相对较弱。该个例不单独作为总体结论依据，但可作为对上文统计结果的定性补充：蒸馏的收益不主要体现为生成更长回答，而在于让回答更贴近实验助教场景中的关键机理、关键症状与排查语境。

== 局限

系统在确定性链路、检索契约与边缘部署上的能力已获实测支撑，主要不足在于：评测题量仅 30 题、教师列仅作质量上限参考；蒸馏学生在无接地兜底路径上的语言质量受参数规模所限；端到端“真实照片→正确诊断”的成功率仍需在更大真板集上统计。
...
#{
  ...
  文献(
    1,
    "NAGEL L W, PEDERSON D O. SPICE: Simulation Program with Integrated Circuit Emphasis[R]. Berkeley: University of California, Electronics Research Laboratory, 1973.",
  )
  文献(
    2,
    "LIU H, LI C, WU Q, et al. Visual instruction tuning[C]// Advances in Neural Information Processing Systems (NeurIPS), 2023.",
  )
  文献(
    3,
    "ZHONG R, DU X, KAI S, et al. LLM4EDA: Emerging progress in large language models for electronic design automation[J/OL]. (2023)[2026-06-30]. https://arxiv.org/abs/2401.12224.",
  )
  文献(
    4,
    "CHAUDHURI J, THAPAR D, CHAUDHURI A, et al. SPICED: Syntactical bug and Trojan pattern identification in A/MS circuits using LLM-enhanced detection[C]// IEEE Physical Assurance and Inspection of Electronics (PAINE), 2024.",
  )
  文献(
    5,
    "CHEN L, SUN W, XIE H, et al. Enhancing large language model-based systems for end-to-end circuit analysis problem solving[J/OL]. (2025)[2026-06-30]. https://arxiv.org/abs/2512.10159.",
  )
  文献(
    6,
    "SHI Y, TAO Z, GAO Y, et al. AMSnet 2.0: A large AMS database with AI segmentation for net detection[J/OL]. (2025)[2026-06-30]. https://arxiv.org/abs/2505.09155.",
  )
  文献(
    7,
    "JOCHER G, CHAURASIA A, QIU J. Ultralytics YOLO[CP/OL]. (2023)[2026-06-30]. https://github.com/ultralytics/ultralytics.",
  )
  文献(
    8,
  ...
  文献(
    9,
    "HAGBERG A A, SCHULT D A, SWART P J. Exploring network structure, dynamics, and function using NetworkX[C]// Proceedings of the 7th Python in Science Conference (SciPy), 2008: 11-15.",
  )
  文献(
    10,
  ...
  文献(
    11,
    "LEWIS P, PEREZ E, PIKTUS A, et al. Retrieval-augmented generation for knowledge-intensive NLP tasks[C]// Advances in Neural Information Processing Systems (NeurIPS), 2020.",
  )
  文献(
    12,
    "Intel Corporation. OpenVINO Toolkit: Open Visual Inference and Neural Network Optimization[CP/OL]. [2026-06-30]. https://github.com/openvinotoolkit/openvino.",
  )
  文献(
    13,
    "HINTON G, VINYALS O, DEAN J. Distilling the knowledge in a neural network[C]// NIPS Deep Learning and Representation Learning Workshop, 2015.",
  )
  ...
  文献(
    17,
    "HU E J, SHEN Y, WALLIS P, et al. LoRA: Low-rank adaptation of large language models[C]// International Conference on Learning Representations (ICLR), 2022.",
  )
  文献(
    18,
    "LIN J, TANG J, TANG H, et al. AWQ: Activation-aware weight quantization for LLM compression and acceleration[C]// Proceedings of Machine Learning and Systems (MLSys), 2024.",
  )
}
