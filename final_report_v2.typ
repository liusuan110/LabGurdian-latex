...
该部分流程的核心产物是结构化网表。一个保留元件、引脚、网络、节点索引与板型拓扑的结构化对象，被交互端、验证模块与智能体共同消费，亦可导出拓扑级 SPICE 网表（导线已隐式合并，元件取值暂以占位）。它同时服务于电气比较与可视化定位——选中某网络即可高亮其全部相关孔位（结构化网表及其拓扑级 SPICE 导出的实例见#ref(<fig:netlist>)）。
...
纯图比对需要一份确定的参考电路，但同一教学拓扑常有多种正确的实现方式，以单一参考硬比较极易误报。为此系统引入拓扑模板，把每类规范拓扑表达为“偏规范（partial specification）”，以三层语义刻画结构（如#ref(<tab:tier>)所示）：`required` 缺失则不匹配，`optional` 可缺不扣分，`forbidden` 出现即降级或否决；另以变体与重数容纳多种合法实现，并声明参数化不变量（当前视觉管线尚未提取数值，按设计仅声明、执行留待后续）。
