...
这部分主要看蒸馏和量化会不会影响教学解释质量。为减少主观判断，我们引入了自动评分。具体做法是让 DeepSeek-V3 作为独立评委，在 `temperature=0` 的设置下，对每道回答从正确性、教学性、简洁性、格式结构和依据安全性五个维度分别打 1 到 5 分。这种方式更细，也更容易复现。作为对照，我们还用了合规校验规则再打一次分，比如是否先提醒安全、有没有提到关键故障点、步骤是否完整。两套自动评分的整体趋势是一致的，只是 LLM-Judge 能看得更细。
