...
#let 竞赛年份 = "2026"
#let 报告题目 = "LabGuardian：一种面向电子实验教学的神经—符号边缘电路诊断系统"
#let 英文题目 = "LABGUARDIAN: A NEURO-SYMBOLIC EDGE SYSTEM FOR BREADBOARD WIRING DIAGNOSIS IN ELECTRONICS LABORATORY EDUCATION"
#let 学生姓名 = ""
#let 指导教师 = ""
#let 参赛学校 = ""

#let 中文摘要 = [
  高校电子实验中，学生常出现“原理图正确但实物接错”的接线错误。这类错误难以从外观直接发现，排查依赖教师逐项检查，而电子实验课师生比偏高，教师难以及时照顾全部学生，极性反接和电源短路还可能损坏器件并带来安全风险。本文研究面向电子实验教学的自动接线诊断与解释方法，目标是在不依赖云端、可在边缘设备本地运行的前提下，给出可审计且可解释的诊断结果。所提系统 LabGuardian 不让大模型直接判定电路对错，而是采用神经感知与符号判断相分离的设计。系统以面包板俯视照片为输入，先用自建引脚级数据集训练的关键点模型完成引脚检测，再将检测到的引脚对齐到最近孔位并结合面包板导通规则，把图像重构为引脚级电气网表。随后由确定性图比对算法承担对错判断，输出带证据引用的结构化错误码。最后由端侧小模型仅依据检索到的证据生成自然语言解释，并由校验器拦截缺乏依据的陈述。其中，基于关键点的引脚级网表重构、由确定性算法判断对错而由小模型只在给定证据范围内解释的分工设计，以及面向边缘部署的双教师蒸馏与 INT4 量化，是本文的主要创新。在 Intel DK-2500 边缘平台上，视觉模型的引脚关键点检测经 NPU INT8 推理达到 13.37 ms 延迟与 74.7 img/s 吞吐，增量功耗约 4.12 W。蒸馏并量化后的 INT4 学生模型约 0.88 GB，解码吞吐约 23 token/s，推理时整封装功率约 10.2 W，空载约 4.4 W。在六个示范故障场景上，将小模型限定为只依据检索到的证据作答，使其解释正确对应到真实故障的比例由 0/6 提升至 6/6。LLM-as-Judge 五维自动评分显示，蒸馏 INT4 学生模型综合得分 3.37/5，高于未蒸馏基座的 3.23/5，表明蒸馏与 INT4 量化未见教学回答质量下降。整系统可在 8 GB 内存的边缘设备上完成诊断与解释。
]
#let 中文关键词 = ("面包板接线诊断", "神经—符号", "可审计解释", "知识蒸馏", "边缘智能")

#let 英文摘要 = [
  In undergraduate electronics laboratories, students often build circuits that are schematically correct but physically miswired. Such wiring errors are hard to spot from appearance and must be checked item by item, yet the high student-to-teacher ratio makes timely help difficult, and reversed polarity or a shorted supply can damage components and raise safety concerns. This paper studies automatic diagnosis and explanation of breadboard wiring for laboratory teaching, aiming to produce auditable and explainable feedback that runs locally on an edge device without cloud access. The proposed system, LabGuardian, does not let a large model judge circuit correctness directly. Instead it separates neural perception from symbolic judgement. It takes a top-view breadboard photo as input, detects pin keypoints with a model trained on a self-built pin-level dataset, aligns each detected pin to its nearest hole, and reconstructs a pin-level electrical netlist using breadboard connectivity rules. A deterministic graph-matching engine then makes the correctness judgement and emits structured error codes with evidence references. Finally an on-device small model generates a natural-language explanation using only the retrieved evidence, and a verifier rejects statements that are not supported by that evidence. The main contributions are the keypoint-based pin-level netlist reconstruction, a division of labour in which a deterministic algorithm decides correctness while the small model explains only within the given evidence, and dual-teacher distillation with INT4 quantization for edge deployment. On the Intel DK-2500 edge platform, the pin-keypoint detection reaches 13.37 ms latency and 74.7 img/s throughput under NPU INT8 inference, with about 4.12 W incremental power. The distilled INT4 student model is about 0.88 GB and decodes at about 23 token/s, with a package power of about 10.2 W during inference and about 4.4 W at idle. Restricting the small model to answer only from the retrieved evidence raises, across six demonstration scenarios, the share of its explanations that correctly point to the real fault from 0/6 to 6/6. A five-dimension LLM-as-Judge evaluation gives the distilled INT4 student a score of 3.37/5, above the un-distilled baseline at 3.23/5, indicating that distillation and INT4 quantization do not degrade teaching-answer quality. The whole system runs diagnosis and explanation on an edge device with 8 GB of memory.
]
#let 英文关键词 = (
...
在技术选型上，服务端以 Python 与 FastAPI 构建异步接口，视觉识别采用 YOLO 系列模型经 OpenVINO 适配英特尔异构硬件。电气比对基于图论库实现确定性图同构与子图匹配，知识检索使用本地多通道知识库。端侧解释模型由开源中文基座（Qwen2.5-1.5B，详见第五章）蒸馏量化得到，交互端以画布叠加方式呈现识别与诊断结果。
...
在真实面包板上，整图关键点检测的输出经组件归属与孔位映射汇聚为引脚级结构化事实，一例端到端贯通如#ref(<fig:e2e>)所示。
...
诊断智能体统一编排诊断、概念讲解、操作指导与混合问答四类教学意图：共用同一张状态图，仅按意图切换工具白名单、校验规则与回答模板（如#ref(<fig:intent>)所示）；工具选择进一步取决于错误族（如短路类调用网表追踪与板型查询），智能体据此动态选工具#引[10]，而非把所有知识与历史塞入上下文。意图判定以确定性关键词分类为主：端侧小模型做四类意图分类的准确率与延迟均不及零延迟关键词路由，故路由交给关键词分类、小模型只负责生成（实测消融见本章三组消融实验一节）。
...
本章前述的诊断智能体在没有可用大模型时会退回确定性模板；算力允许时，则由大模型把结构化诊断结果改写成更容易理解的讲解。为把这一环节放到边缘端本地运行、减少对云端的依赖，本项目采用双教师蒸馏#引[13]训练了一款面向教学问答的小模型，并完成 OpenVINO INT4 导出与初步评测。

蒸馏数据不是让教师模型自由发挥生成的，而是严格建立在系统已经固定好的结构化证据上。项目选用本地部署的 Qwen3-32B#引[14]和云端的 DeepSeek-V3#引[15]作为双教师，让它们在同一份固定证据上分别作答，把回答内容和可核查的事实对应起来，尽量减少幻觉。候选问题围绕典型电路、常见故障和学生常问的问题自动扩展，再结合人工与规则过滤掉不贴近课堂场景的样本；教师回答也不会直接拿来训练，只保留有明确依据、没有被降级为“证据不足”、长度合适且表达基本一致的样本。最终从 4990 条候选教师输出中保留 3466 条高质量样本，并构建为 3450 条正式 SFT 数据（候选留存率约 69.1%）。学生模型以 Qwen2.5-1.5B-Instruct#引[16]为基座，经 LoRA#引[17]微调并合并权重后，再进行 INT4 量化#引[18]并导出为 OpenVINO 部署模型，合并后约 3.1 GB、INT4 约 0.88 GB，拷贝到板端即可运行，全程无需外网。
...
#figure(
  ...
  #diagram(
    ...
    node((0, 1.5), align(center)[固定结构化证据\ #text(7.5pt, fill: luma(85))[场景 · 错误码\ 故障案例 · 工具结果\ 风险等级]\ #text(weight: "bold")[检索契约]], stroke: 1.3pt + black, fill: luma(240), name: <ev>),
    ...
    node((3.9, 1.45), text(7.5pt)[筛样], shape: fletcher.shapes.trapezium.with(angle: -22deg), fill: luma(240), name: <filt>),
    ...
    edge(<ev>, (0, 3.9), (5.9, 3.9), <ov>, "--|>", stroke: luma(110),
      label: text(7.5pt, fill: luma(90))[训练即部署 · 同一检索契约], label-pos: 0.5, label-side: center),

    node((3.9, 3.05), text(7pt, fill: luma(100))[引用合规 · 长度 · 未降级　留存 69.1%], stroke: none, inset: 1pt),
  )],
  caption: [基于固定证据的双教师检索增强蒸馏流程：本地 Qwen3-32B 与云端 DeepSeek-V3 在同一份固定结构化证据上作答，候选池经筛样（引用合规、长度达标、未降级为“证据不足”）得到 3450 条 SFT 数据，用以微调 Qwen2.5-1.5B 学生模型并经 INT4 量化部署；训练数据生成与板端部署共用同一检索契约],
) <fig:distill-record>
...
== 三组消融实验，分别看契约、意图和蒸馏量化的作用

本节安排了三组消融实验。每组只改一个条件，其余保持不变，用来分别看检索契约、意图路由和蒸馏量化到底带来了什么提升。

=== 契约消融，同一模型看是否走契约路径

这组实验用的是同一款 INT4 学生模型，只比较它是否走完整契约路径。我们在六个示范场景里各注入一个典型故障，让同一道学生问题分别走“基座模型直答”和“完整契约路径”，结果见#ref(<tab:bare-contract>)。不走契约时，模型六个场景都说不清涉事故障器件；走契约后，六个场景都能对准场景、引用错误码和涉事器件，危险场景还会先提醒断电，并且都能通过校验。这说明这里的提升主要来自检索契约，而不是单纯靠模型自己发挥。

#figure(
  三线表(
    columns: (1.2fr, 1.4fr, 1.4fr),
    align-cells: (left, center, center),
    header: ([评估维度], [基座模型直答], [完整契约路径]),
    rows: (
      ([命中涉事故障器件], [0 / 6], [6 / 6]),
      ([引用当前错误码], [0 / 6], [6 / 6]),
      ([引用涉事元件], [0 / 6], [6 / 6]),
  ...
  caption: [基座模型 vs 检索契约，六示范场景诊断接地能力对照（同一 INT4 学生模型）],
) <tab:bare-contract>
...
这部分主要看蒸馏和量化会不会影响教学解释质量。为减少主观判断，我们引入了自动评分。具体做法是让 DeepSeek-V3 作为独立评委，在 `temperature=0` 的设置下，对每道回答从正确性、教学性、简洁性、格式结构和接地安全性五个维度分别打 1 到 5 分。这种方式更细，也更容易复现。作为对照，我们还用了确定性校验规则再打一次分，比如是否先提醒安全、有没有提到关键故障点、步骤是否完整。两套自动评分的整体趋势是一致的，只是 LLM-Judge 能看得更细。
...
项目进一步用同一套 30 题，对基座模型、蒸馏 `FP`、蒸馏 `INT4` 和教师上限做了横向对比，结果见#ref(<fig:p0-four-judge>)。综合得分的排序很清楚，教师 3.93，`INT4` 3.37，`FP` 3.33，基座 3.23。这个结果说明，蒸馏后的学生模型整体上优于基座模型，量化也没有把效果拉低。再看细项，当前最弱的一项还是教学启发性，这和回答长度偏短有关；但在接地安全性上，蒸馏模型相比基座有稳定提升。
...
#figure(
  三线表(
    ...
    rows: (
      ...
      ([检索契约], [基座模型 vs 契约路径], [故障接地 0/6 → 6/6，校验通过 6/6]),
