...
围绕上述需求，本项目的主要工作与创新点如下：（1）提出面向噪声感知输入的引脚级电气网表重构方法，以关键点检测与孔位吸附替代元件级目标检测，将识别粒度下沉至引脚与孔位；（2）以确定性图比对承担对错判断、智能体仅在受约束证据内生成解释，并以不可绕过的确定性校验器拦截无依据陈述，构成“神经—符号”的可审计诊断链；（3）面向边缘部署，采用双教师蒸馏与 INT4 量化得到端侧教学解释模型，并在 Intel DK-2500 上以 NPU、iGPU、CPU 异构分工实现低功耗本地推理；（4）通过三组正交消融与端侧实测，分别验证检索约束、模型蒸馏与量化各自的贡献。

== 核心创新与诊断三链路

本项目的独特立意不在于简单叠加“视觉模型 + 大语言模型”，而在于把诊断拆成三条互相制约、逐级递进的链路——视觉识别、网络搭建与情景解释。如#ref(<fig:three-chains>)所示，前两链以确定性算法产出可审计的结构化事实，第三链才由受约束的小模型把事实改写为教学语言。三者共同形成“全图视觉识别—与参考电路逻辑比对—受约束解释”的闭环，既体现人工智能能力，又不把安全敏感的正误判定交给不可审计模型。
...
本章是“神经—符号”中的符号侧，它不依赖任何神经网络，对错判断由图论算法以确定、可复现、可审计的方式给出，从根本上杜绝了在安全敏感的“对错”问题上产生幻觉。
  ...
  [#set text(size: 8.5pt)
    #diagram(
      ...
      node(
        ...
        align(
          center,
        )[固定结构化证据\ #text(7.5pt, fill: luma(85))[场景 · 错误码\ 故障案例 · 工具结果\ 风险等级]\ #text(weight: "bold")[检索契约]],
        stroke: 1.3pt + black,
        fill: luma(240),
        name: <ev>,
      ),
      ...
      edge(
        ...
        "--|>",
        stroke: luma(110),
        label: text(7.5pt, fill: luma(90))[训练即部署 · 同一检索契约],
        label-pos: 0.5,
        label-side: center,
      ),
