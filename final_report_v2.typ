...
为使教学解释环节能够在边缘端本地运行，本文采用基于固定诊断证据的双教师响应蒸馏方法训练端侧小语言模型。需要强调的是，语言模型不参与电路正误判定；电路连接关系、错误类型和证据引用均由前述确定性图比对模块给出，端侧模型只学习如何在给定证据范围内组织语言、面向学生生成教学解释。若生成内容超出既有证据边界，则由证据一致性校验拦截并退回模板解释。
...
#figure(
  三线表(
    ...
    header: ([字段], [样例内容（节选）]),
    rows: (
      ([system], [你是 LabGuardian 课堂实验助教；回答必须严格依据给定 fixed evidence，不得编造器件、引脚、节点、网表或实验现象]),
      ([instruction], [我信号源和示波器没共地，RC 波形乱跳，是不是地线没接好？]),
      ([input], [`scene_id=exp_first_order_rc`；`risk_level=danger`；`error_codes=COMPONENT_SHORTED_SAME_NET`；`finding: component=SCOPE_GND, pin=gnd_clip`；并附带允许工具、证据引用要求与工具检索结果]),
      ([output], [先给结论，再给依据，再给修改步骤；例如先提醒断电，再说明示波器地夹同网短接风险，最后给出复查接地点与共地关系的操作建议]),
    ),
  ),
...
#figure(
  三线表(
    ...
    rows: (
      ...
      ([固定证据], [`scene_id=exp_first_order_rc`；`risk_level=warning`；要求先断电，优先检查 `C1` 是否悬空或接触不良，并检查节点连通与接地参考]),
      ([端侧真实输出], [先断电，检查 `C1` 是否悬空，然后检查输出端是否接错节点，再检查输入与地是否共参考。操作顺序：先断电，再检查元件接触、孔位和节点连通]),
      ([证据一致性校验], [通过。输出命中“先断电 / `C1` 悬空 / 节点连通”三项关键证据，未新增不存在的器件、引脚或错误码；因此允许直接返回学生端]),
    ),
  ),
...
与既有工作相比，本系统主要有三点不同。其一，面向必然带有干扰的真实照片输入，而非理想的干净网表；其二，以神经—符号的思路把对错判断交给可审计的符号系统、把语言表达交给受约束的小模型；其三，让训练阶段与板端部署使用同一套检索约束和固定证据边界，再配合证据一致性校验，把“防止无依据扩展”做成工程上可审计的约束而非提示技巧。这样一个十亿级小模型也能在对正确性要求较高的教学场景中、于边缘设备上稳定运行。
