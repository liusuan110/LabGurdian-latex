...
视觉链路的可靠性首先取决于训练数据，而本场景对数据的要求相当特殊：既非电路原理图层面的符号数据，亦非印制电路板（PCB）层面的器件数据，而是真实面包板照片上、精确到引脚与孔位的细粒度标注，且须覆盖真实拍摄中普遍存在的遮挡、反光与视角差异。据我们调研，当前学术界缺乏严格符合这一情景的公开数据集——已有电路类数据集多停留在原理图识别、PCB 缺陷检测或干净网表层面#引[6]，鲜有在“教学面包板 + 引脚级关键点 + 孔位映射 + 真实噪声”四个维度上同时满足本任务者。为此，本项目自行采集并标注了一套专用数据集，它构成本系统视觉感知的基础，亦相对于既有公开资源具有独创性。
...
最终的 pose 训练运行采用 Ultralytics YOLOv8s-pose#引[7]，输入尺寸 960、batch size 为 8、训练 100 轮，启用默认优化器与常规颜色/平移/缩放增强，末 10 轮关闭 mosaic。训练日志显示，模型在第 100 轮的验证集上，组件检测框（B）指标为 precision=0.991、recall=0.989、mAP50=0.991、mAP50-95=0.786，引脚关键点（P）指标为 precision=0.955、recall=0.954、mAP50=0.947、mAP50-95=0.829（其中 B 指检测框指标、P 指引脚关键点指标）。虽然课堂真实拍摄仍会受到遮挡、模糊与反光影响，但上述验证集指标已说明模型能够较稳定地同时完成组件定位与引脚关键点回归，为后续孔位映射与拓扑重构提供可用的视觉前提。测试集未参与任何训练与模型选择决策，留作后续端到端系统级评测的独立样本。
...
诊断智能体统一编排四类教学意图——诊断、概念讲解、操作指导与混合问答。四类意图共用同一张状态图，仅按意图切换工具白名单、回答校验规则与回答模板，如#ref(<fig:intent>)所示。工具选择进一步取决于错误所属的错误族，每族对应一份白名单——如短路类调用网表追踪、板型查询与热力图叠加。智能体据此动态选工具#引[10]，而非把所有技能、知识与历史一并塞入上下文。意图判定采用确定性关键词分类为主：经实测，端侧十亿级小模型直接做四类意图分类，准确率与延迟均不及确定性关键词路由，故系统以零延迟、可解释的关键词分类承担路由，小模型只负责生成（详见第七章）。
...
为避免“训练一套知识、部署另一套”的不一致，系统对智能体的检索增强生成（RAG）#引[11]施加可审计约束：合法知识源仅有教学场景库、故障案例库、器件手册库与电路知识库四个通道，另加工位状态、流水线快照与参考电路等结构化事实通道；早期基于通用向量库的文档检索被禁止进入主链路，以杜绝分布漂移、云端依赖与内容污染。检索同样失败拒止：板端无可用大模型时退回确定性模板，宁可少答也不让错误默认值污染结论。
...
在 `P0` 可用性验证之外，项目进一步补做了本地 `FP` merged 模型与部署版 `INT4` 学生模型的 `P0.5` 质量对比。两者使用同一套 30 题固定题集、同一套回答约束与同一套人工 rubric，仅更换推理后端；结果如#ref(<fig:p05-quality>)与#ref(<tab:p05-quality>)所示：本地 `FP` merged 模型的平均得分率为 64.4%，部署版 `INT4` 为 71.1%，二者差值为 -6.7 个百分点；两者通过率均为 80.0%，但 `INT4` 的满分率仍高出 13.3 个百分点。即在当前固定题集与人工评分口径下，部署版 `INT4` 相对本地 `FP` 基线未见明显质量退化，且在满分率与整体完成度上略优。

进一步看分意图表现，`FP` 仅在 `diagnostic` 类题目上优于 `INT4`（71.8% vs 59.0%，高出 12.8 个百分点）；而在 `concept_tutor`、`lab_guidance` 与 `mixed` 三类题目上，部署版 `INT4` 分别高出 25.0、6.7 与 33.3 个百分点。结合重跑后的平均时延看，本地 `FP` 每题平均生成耗时约 5.8 秒，而部署版 `INT4` 约为 7.7 秒，二者已经处于同一量级，且 `FP` 在本轮重跑中略快于 `INT4`。这说明在当前任务口径下，质量差异与推理时延并不简单由“是否量化”单独决定，而更受具体推理后端、生成收敛性与实现链路影响；因此，`INT4` 的主要优势仍应表述为在当前固定题集上保持更高整体得分与更高满分率，而非绝对更低时延。

#figure(
  image("board_data/p0_5_fp_vs_int4.png", width: 94%),
  caption: [P0.5 本地 `FP` merged 与部署版 `INT4` 学生模型对比结果：左图比较 30 题固定题集上的平均得分率、通过率与满分率，右图比较单题平均生成时延。当前实现链路下，`INT4` 在平均得分率与满分率上均优于本地 `FP`；而在重跑后的时延统计中，两者已处于同一量级，`FP` 略快于 `INT4`。],
) <fig:p05-quality>
...
#figure(
  ...
  caption: [P0.5 量化前后质量与时延对比摘要。这里的 `FP` 为本地 HuggingFace merged 推理链路，`INT4` 为 OpenVINO 部署链路；该对比可用于验证“当前部署版 `INT4` 未出现明显质量退化”，但不等同于板端 `FP16` 真机部署结果。],
) <tab:p05-quality>

=== 四模型横向矩阵

在完成 `P0` 与 `P0.5` 之后，项目进一步把同一套 30 题固定题集扩展为完整四模型横向矩阵：未蒸馏基座 `Qwen2.5-1.5B-Instruct`、蒸馏学生 `FP`、蒸馏学生 `INT4`，以及教师上限。四列均使用同一题集、同一短答约束与同一 3 点 rubric 统计平均得分率、通过率与满分率；结果如#ref(<fig:p0-four-model>)与#ref(<tab:p0-four-model>)所示。

从质量侧看，未蒸馏基座、本地 `FP`、部署版 `INT4` 与教师上限的平均得分率依次为 66.7%、64.4%、71.1% 与 92.2%。需指出，基座 `Qwen2.5-1.5B-Instruct` 本即具备相当通用问答能力，在 30 题规模下基座、`FP` 与 `INT4` 之间数个百分点的差异尚处于统计噪声范围；本系统的蒸馏目标并非在裸答题得分上超越基座，故此指标与基座持平本属预期。蒸馏的价值不在裸能力，而在于使小模型稳定产出受契约约束、可接地且含安全前置的教学解释，其事实正确性由不可绕过的确定性校验器强制保证、与模型规模解耦（参见前文契约消融，裸模型 0/6 → 契约 6/6）。在此定位下，本矩阵给出两点确定结论：其一，部署版 `INT4` 相对本地 `FP` 未见质量退化（满分率反高 13.3 个百分点），量化未损害可用性；其二，教师上限较 `INT4` 高 21.1 个百分点，界定了在保持边缘可部署性前提下教学表达仍可追赶的空间。

从时延侧看，本地基座、`FP` 与部署版 `INT4` 的单题平均生成时延分别约为 4.26、5.80 与 7.73 秒，教师经云端 API 约 1.32 秒。三条本地链路处于同一量级，其差异主要受推理后端与生成收敛状态影响；由于教师走云端 API、学生分别走本地 HuggingFace 与 OpenVINO 链路，该列反映的是当前实现下的端到端响应成本，而非不同参数规模之间的纯推理速度对比。综上，四模型矩阵明确了三点：部署版 `INT4` 具备可用的教学解释质量，相对本地 `FP` 未见明显退化，且与教师上限尚存可追赶的质量缺口。

#figure(
  image("board_data/p0_four_model_matrix.png", width: 94%),
  caption: [P0 固定题集四模型横向矩阵：左图为未蒸馏基座、蒸馏学生 `FP`、蒸馏学生 `INT4` 与教师上限的平均得分率，右图为当前实现链路下的单题平均生成时延。在 30 题规模下，部署版 `INT4` 与基座、本地 `FP` 大体相当（差异在小样本噪声内）、量化未见退化，而教师上限仍保留明显质量优势。],
) <fig:p0-four-model>
...
#figure(
  ...
  caption: [P0 四模型横向质量矩阵摘要。四列共用同一 30 题固定题集与同一 rubric；其中教师列作为质量上限参考，时延数字受 API 后端与本地部署环境差异影响，仅用于当前实现链路下的综合响应成本比较。],
) <tab:p0-four-model>

== 局限与后续方向

综合上述实验，系统在确定性链路、检索契约与边缘部署上的能力已获实测支撑；主要不足在于：其一，虽然当前已经完成四模型横向质量矩阵，但其题量仍仅为 30 题、教师列亦主要用于质量上限参考，后续仍需扩展更大规模 gold 评测集并补做更严格的人审复核；其二，蒸馏学生在无接地兜底路径上仍可能生成质量欠佳内容（已由 llm_fallback 标注与校验器约束在事实层面，但语言质量受参数规模所限）；其三，端到端“真实照片→正确诊断”的成功率仍需在更大真板集上统计。
