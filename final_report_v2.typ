...
#figure(
  三线表(
    ...
    rows: (
      ...
      ([数据规模], [项目自行拍摄并标注的真实面包板照片 1000 余张，涵盖多角度、光照与遮挡变体]),
...
蒸馏数据并非由教师模型自由生成，而是约束在系统已固化的结构化证据之上：以本地部署的 Qwen3-32B 与云端的 DeepSeek-V3 构成双教师，在同一份冻结证据上分别作答并据此构建监督微调数据，从而把教师的语言能力与系统可验证的事实绑定，抑制教师幻觉被放大的风险。学生模型以 Qwen2.5-1.5B-Instruct 为基座，经 LoRA 微调、权重合并后导出为 OpenVINO INT4：合并后完整模型约 3.1 GB、INT4 约 0.88 GB，部署门槛明显低于仅作质量上限参考的 4B 级教师模型，且随项目代码拷贝至板端即可运行、全程无需外网。
...
#figure(
  三线表(
    ...
    rows: (
      ...
      ([双教师生成], [Qwen3-32B 与 DeepSeek-V3 在同一证据上作答], [教师只能引用已给证据]),
      ([学生微调], [Qwen2.5-1.5B-Instruct + LoRA 监督微调并合并权重], [训练样本与部署检索契约一致]),
