...
视觉链路的可靠性首先取决于训练数据，而本场景需要的既非原理图符号数据、亦非印制电路板（PCB）器件数据，而是真实面包板照片上精确到引脚与孔位的细粒度标注。经调研，已有电路类数据集多停留在原理图识别、PCB 缺陷检测或干净网表层面#引[6]，鲜有在“教学面包板 + 引脚级关键点 + 孔位映射 + 真实干扰”四个维度同时满足本任务者。为此本项目自行采集并标注了一套专用数据集，构成本系统视觉感知的基础，也是既有公开资源所缺少的。
...
为避免“训练一套知识、部署另一套”的不一致，系统对智能体的检索增强生成（RAG）#引[11]施加可审计约束，合法知识源仅有教学场景库、故障案例库、器件手册库与电路知识库四个通道，另加工位状态、流水线快照与参考电路等结构化事实通道（如#ref(<tab:retrieval-contract>)所示）。早期基于通用向量库的文档检索仅保留给后台调试，禁止进入主链路或蒸馏管线，以避免分布漂移、云端依赖与内容污染。
...
上述约定让蒸馏训练与板端部署保持一致，蒸馏数据生成与板端部署使用同一套代码、知识与场景白名单，并由起飞前自检脚本校验两端等价。它把“可用知识的边界”做成可审计的工程约束，场景白名单按拓扑过滤器件手册，使反相放大器的问题不会召回无关的定时器芯片；故障案例仅在错误码命中时返回，召回键正是第四章图比对输出的错误码词表。检索缺失时系统 fail-closed，蒸馏样本拒绝生成、部署端回退确定性模板，宁可少答也不让错误默认值污染结论。这一取向与近年电子设计自动化语言模型“以确定性结构约束生成”的方向一致#引[3]。
