...
围绕上述需求，本项目的主要工作与创新点如下：（1）提出面向噪声感知输入的引脚级电气网表重构方法，以关键点检测与孔位吸附替代元件级目标检测，将识别精度下沉至引脚与孔位；（2）以确定性图比对承担对错判断、智能体仅在受约束证据内生成解释，并以不可绕过的确定性校验器拦截无依据陈述，构成“神经—符号解耦”的可审计诊断链；（3）面向边缘部署，采用双教师蒸馏与 INT4 量化得到端侧教学解释模型，并在 Intel DK-2500 上以 NPU、iGPU、CPU 异构分工实现低功耗本地推理；（4）通过三组正交消融与端侧实测，分别验证检索契约、模型蒸馏与量化各自的贡献。
...
总体架构要回答的核心问题是，如何既用上大模型的语言能力，又不让它参与安全敏感的对错判定。LabGuardian 的答案是以分层把“事实生产”与“话术生成”在结构上彻底隔开。系统采用交互端与服务端分离：交互端负责图片上传、参数与参考电路选择、结果可视化、人工修正与智能体对话；服务端负责接口接入、图像处理、模型推理、孔位映射、拓扑构建、逻辑比较、课堂状态、知识检索、智能体编排与遥测推流。整体可概括为“交互层—API 服务层—流水线事实层—领域规则层—Agent 与知识解释层—基础支撑层”六个层次，各层次的职责如#ref(<fig:arch>)所示。由此“事实生产”与“话术生成”彻底解耦——下游的自然语言解释永远以上游可证伪的结构化事实为唯一依据。
...
电气诊断要求识别精度下沉到引脚级，而常规目标检测无法回答“某引脚究竟插在哪个孔”。直觉方案是先裁剪单个元件框、再在框内回归引脚，但实践发现裁剪会把引脚切到框外并丢失周边上下文，导致系统性漏检。因此系统改为整图关键点检测：在俯视图上以 YOLO-Pose 一次性回归全部引脚关键点，再依类别与边界框的几何约束把引脚归属到对应元件，既避免裁剪误差，又保留完整引脚证据。
...
诊断智能体统一编排诊断、概念讲解、操作指导与混合问答四类教学意图：共用同一张状态图，仅按意图切换工具白名单、校验规则与回答模板（如#ref(<fig:intent>)所示）；工具选择进一步取决于错误族（如短路类调用网表追踪与板型查询），智能体据此动态选工具#引[10]，而非把所有知识与历史塞入上下文。意图判定以确定性关键词分类为主：经实测，端侧小模型做四类意图分类的准确率与延迟均不及关键词路由，故路由交给零延迟、可解释的关键词分类，小模型只负责生成。
...
为量化上述分工，项目以 turbostat 按 0.25 秒间隔采样 RAPL 能量计，并在板端以输入分辨率 960 单进程串行采集 60 至 1153 次推理统计，测得 YOLOv8s-pose 关键点检测在 CPU、iGPU、NPU 三种单元上的延迟、吞吐与功耗。其功耗时序如#ref(<fig:power-ts>)所示：NPU 推理期间 CPU 核心与 iGPU 功率曲线全程贴近空闲基线（约 4.42 W），说明 NPU 几乎不占用其他单元资源，为“NPU 负责视觉、iGPU 负责诊断、CPU 负责控制”的三路异构分工提供了硬件层面的可行性。
...
与原有 3 点人工 rubric 相比，LLM-Judge 自动评分有三点优势：其一，五维 1-5 量尺区分度远超三元命中，能够分辨同一得分率背后的质量差异；其二，自动评分完全可复现（temperature=0），消除了作者自评的主观偏差；其三，按维度与意图的细粒度分解直接指向优化方向——诊断排查类题目与教学启发性维度是后续蒸馏数据增强与提示约束优化的重点。
