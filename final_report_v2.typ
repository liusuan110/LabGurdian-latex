...
#figure(
  ...
  caption: [主业务流程：上排为自动诊断主链；下方为人机协同修正回路——视觉结果不准时，人工修正后从拓扑重构起重算，不重跑视觉检测],
) <fig:pipeline>
...
为量化上述分工，项目以 turbostat 按 0.25 秒间隔采样 RAPL 能量计，并在板端以输入分辨率 960单进程串行采集 60 至 1153 次推理统计，测得 YOLOv8s-pose 关键点检测在 CPU、iGPU、NPU 三种单元上的延迟、吞吐与功耗。其功耗时序如#ref(<fig:power-ts>)所示：NPU 推理期间 CPU 核心与 iGPU 功率曲线全程贴近空闲基线（约 4.42 W），说明 NPU 几乎不占用其他单元资源，为“NPU 负责视觉、iGPU 负责诊断、CPU 负责控制”的三路异构分工提供了硬件层面的可行性。
...
从质量侧看，未蒸馏基座、本地 `FP`、部署版 `INT4` 与教师上限的平均得分率依次为 66.7%、64.4%、71.1% 与 92.2%。需指出，基座本即具备相当通用问答能力，30 题规模下三者数个百分点的差异尚在统计噪声范围；蒸馏目标并非在裸答题得分上超越基座，其价值在于使小模型稳定产出受契约约束、可接地且含安全前置的教学解释，事实正确性由不可绕过的校验器保证、与模型规模解耦。本矩阵给出两点结论：部署版 `INT4` 相对本地 `FP` 未见质量退化（满分率反高 13.3 个百分点）；教师上限较 `INT4` 高 21.1 个百分点，界定了保持边缘可部署性前提下仍可追赶的空间。
...
#figure(
  ...
  caption: [P0 四模型横向矩阵：左为平均得分率，右为当前实现链路下的单题平均生成时延。30 题规模下 `INT4` 与基座、`FP` 大体相当，教师上限仍保留明显优势],
) <fig:p0-four-model>
...
在测试过程中，进一步以一个反相放大器相关的具体问题作定性抽样。对于“我把运放的两个输入接反了，会有什么现象？”这一训练分布内样例，蒸馏后的 `INT4` 学生模型能够较快聚焦到“输出饱和到电源轨附近”这一课堂实验中最关键的异常表现，并把回答收束在故障机理与直接现象上。未蒸馏基座模型虽然同样判断“电路无法正常工作”，但后续表述更偏一般性说明，范围较宽，对实验语境下核心症状的聚焦度相对较弱。该个例不单独作为总体结论依据，但可作为对上文统计结果的定性补充：蒸馏的收益不主要体现为生成更长回答，而在于让回答更贴近实验助教场景中的关键机理、关键症状与排查语境。
