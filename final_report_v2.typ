...
第二类是纯规则或模板比较。它借助图同构与引脚角色规则给出确定、可解释的结论#引[2]，但在遇到多余元件、角色标注偏弱、输入输出网络重映射等边界情形时需不断打补丁，易判定过宽或诊断粒度不足。

第三类是端到端大模型直接看图判错。近年多模态大模型展现出强大的视觉理解能力#引[3]，亦有研究进一步将其用于电路网表故障定位与端到端电路分析辅导#引[4-6]；但这类方案多由模型自身承担对错判定、且普遍依赖云端算力，其输出难以审计、易生幻觉，不宜作为正误判定的唯一依据，难以满足教学对安全与可解释的严苛要求。
...
围绕上述需求，本项目的主要工作与创新点如下。（1）提出面向真实拍摄照片的引脚级电气网表重构方法，以关键点检测与孔位吸附替代元件级目标检测，将识别粒度下沉至引脚与孔位；（2）以确定性图比对承担对错判断、智能体仅在受约束证据内生成解释，并以不可绕过的确定性校验器拦截无依据陈述，构成“神经—符号”的可审计诊断链；（3）面向边缘部署，采用双教师蒸馏与 INT4 量化得到端侧教学解释模型，并在 Intel DK-2500 上以 NPU、iGPU、CPU 异构分工实现低功耗本地推理；（4）通过三组正交消融与端侧实测，分别验证检索约束、模型蒸馏与量化各自的贡献。

== 核心创新与诊断三链路

本项目的独特立意不在于简单叠加“视觉模型 + 大语言模型”，而在于把诊断拆成三条互相制约、逐级递进的链路，即视觉识别、网络搭建与情景解释。如#ref(<fig:three-chains>)所示，前两链以确定性算法产出可审计的结构化事实，第三链才由受约束的小模型把事实改写为教学语言。三者共同形成“全图视觉识别—与参考电路逻辑比对—受约束解释”的闭环，既体现人工智能能力，又不把安全敏感的正误判定交给不可审计模型。
...
竞赛平台并非仅作运行服务端的小主机，而是把异构计算能力纳入系统架构。OpenVINO#引[13]适配端侧推理、Level-Zero 与 NPU 驱动栈负责设备调度、遥测通道实时回传利用率与功耗。平台资源与系统负载的对应关系如表#ref(<tab:intel-platform-use>)所示。
...
视觉链路的可靠性首先取决于训练数据，而本场景需要的既非原理图符号数据、亦非印制电路板（PCB）器件数据，而是真实面包板照片上精确到引脚与孔位的细粒度标注。经调研，已有电路类数据集多停留在原理图识别、PCB 缺陷检测或干净网表层面#引[7]，鲜有在“教学面包板 + 引脚级关键点 + 孔位映射 + 真实干扰”四个维度同时满足本任务者。为此本项目自行采集并标注了一套专用数据集，构成本系统视觉感知的基础，也是既有公开资源所缺少的。
...
最终的 pose 训练运行采用 Ultralytics YOLOv8s-pose#引[8]，输入尺寸 960、batch size 为 8、训练 100 轮，启用默认优化器与常规颜色/平移/缩放增强，末 10 轮关闭 mosaic。训练日志显示，模型在第 100 轮的验证集上，组件检测框（B）指标为 precision=0.991、recall=0.989、mAP50=0.991、mAP50-95=0.786，引脚关键点（P）指标为 precision=0.955、recall=0.954、mAP50=0.947、mAP50-95=0.829。上述验证集指标说明模型能较稳定地同时完成组件定位与引脚关键点回归，为孔位映射与拓扑重构提供可用的视觉前提；测试集未参与任何训练与模型选择，留作端到端系统级评测的独立样本。
...
诊断智能体统一编排诊断、概念讲解、操作指导与混合问答四类教学意图，共用同一张状态图，仅按意图切换工具白名单、校验规则与回答模板（如#ref(<fig:intent>)所示）；工具选择进一步取决于错误族（如短路类调用网表追踪与板型查询），智能体据此动态选工具#引[11]，而非把所有知识与历史塞入上下文。意图判定以确定性关键词分类为主，端侧小模型做四类意图分类的准确率与延迟均不及零延迟关键词路由，故路由交给关键词分类、小模型只负责生成（实测消融见本章三组消融实验一节）。
...
为避免“训练一套知识、部署另一套”的不一致，系统对智能体的检索增强生成（RAG）#引[12]施加可审计约束，合法知识源仅有教学场景库、故障案例库、器件手册库与电路知识库四个通道，另加工位状态、流水线快照与参考电路等结构化事实通道（如#ref(<tab:retrieval-contract>)所示）。早期基于通用向量库的文档检索仅保留给后台调试，禁止进入主链路或蒸馏管线，以避免分布漂移、云端依赖与内容污染。
...
上述约定让蒸馏训练与板端部署保持一致，蒸馏数据生成与板端部署使用同一套代码、知识与场景白名单，并由起飞前自检脚本校验两端等价。它把“可用知识的边界”做成可审计的工程约束，场景白名单按拓扑过滤器件手册，使反相放大器的问题不会召回无关的定时器芯片；故障案例仅在错误码命中时返回，召回键正是第四章图比对输出的错误码词表。检索缺失时系统 fail-closed，蒸馏样本拒绝生成、部署端回退确定性模板，宁可少答也不让错误默认值污染结论。这一取向与近年电子设计自动化语言模型“以确定性结构约束生成”的方向一致#引[4]。
...
本章前述的诊断智能体在没有可用大模型时会退回确定性模板；算力允许时，则由大模型把结构化诊断结果改写成更容易理解的讲解。为把这一环节放到边缘端本地运行、减少对云端的依赖，本项目采用双教师蒸馏#引[14]训练了一款面向教学问答的小模型，并完成 OpenVINO INT4 导出与初步评测。

蒸馏数据不是让教师模型自由发挥生成的，而是严格建立在系统已经固定好的结构化证据上。项目选用本地部署的 Qwen3-32B#引[15]和云端的 DeepSeek-V3#引[16]作为双教师，让它们在同一份固定证据上分别作答，把回答内容和可核查的事实对应起来，尽量减少幻觉。候选问题围绕典型电路、常见故障和学生常问的问题自动扩展，再结合人工与规则过滤掉不贴近课堂场景的样本；教师回答也不会直接拿来训练，只保留有明确依据、没有被降级为“证据不足”、长度合适且表达基本一致的样本。最终从 4990 条候选教师输出中保留 3466 条高质量样本，并构建为 3450 条正式 SFT 数据（候选留存率约 69.1%）。学生模型以 Qwen2.5-1.5B-Instruct#引[17]为基座，经 LoRA#引[18]微调并合并权重后，再进行 INT4 量化#引[19]并导出为 OpenVINO 部署模型，合并后约 3.1 GB、INT4 约 0.88 GB，拷贝到板端即可运行，全程无需外网。
...
本文提出并实现了 LabGuardian，一个面向电子实验教学、运行于英特尔边缘平台的神经—符号电路诊断系统。系统以真实拍摄的面包板照片为起点，经关键点检测与孔位吸附重构引脚级电气网表，由确定性图比对承担可审计的对错判断，再由受约束的智能体在不可绕过的校验器约束下生成有依据的解释。教学解释模型经双教师蒸馏与 INT4 量化部署于 DK-2500，以 NPU、iGPU、CPU 异构分工实现低功耗本地推理。实测表明，NPU 以约 4 W 增量功耗达成约 75 img/s 吞吐，端侧学生模型在 iGPU 上以约 10 W、约 23 token/s 完成解释生成，整系统在 8 GB 内存预算内可用。检索约束使同一小模型在六个示范场景上由全部无法指认故障提升为全部正确指出故障。四模型对比显示部署版 INT4（综合 3.37/5）较基座（3.23/5）略优，且未因量化退化，与教师上限（3.93/5）相差约 0.56 分，是可追赶的空间。教学解释的事实正确性由确定性校验器保证，不依赖该得分差。

与既有工作相比，本系统主要有三点不同。其一，面向必然带有干扰的真实照片输入，而非理想的干净网表；其二，以神经—符号的思路把对错判断交给可审计的符号系统、把语言表达交给受约束的小模型；其三，让蒸馏训练与板端部署使用同一套检索约束，再配合不可绕过的校验器，把防幻觉做成工程上可审计的约束而非提示技巧。这一组合使一个十亿级小模型得以在正确性高度敏感的教学场景中、于十瓦级边缘设备上可靠运行。
...
#{
  ...
  // 注：文献按其在正文中首次出现的次序连续编号，[1]–[19] 均已在相应章节以上标形式标注引用。
  文献(
    1,
  ...
  文献(
    2,
    "OHLRICH M, EBELING C, GINTING E, et al. SubGemini: Identifying subcircuits using a fast subgraph isomorphism algorithm[C]// Proceedings of the 30th Design Automation Conference (DAC), 1993: 31-37.",
  )
  文献(
    3,
  ...
  文献(
    4,
    "ZHONG R, DU X, KAI S, et al. LLM4EDA: Emerging progress in large language models for electronic design automation[J/OL]. (2023)[2026-06-18]. https://arxiv.org/abs/2401.12224.",
  )
  文献(
    5,
  ...
  文献(
    6,
    "CHEN L, SUN W, XIE H, et al. Enhancing large language model-based systems for end-to-end circuit analysis problem solving[J/OL]. (2025)[2026-06-18]. https://arxiv.org/abs/2512.10159.",
  )
  文献(
    7,
    "SHI Y, TAO Z, GAO Y, et al. AMSnet 2.0: A large AMS database with AI segmentation for net detection[J/OL]. (2025)[2026-06-18]. https://arxiv.org/abs/2505.09155.",
  )
  文献(
    8,
    "JOCHER G, CHAURASIA A, QIU J. Ultralytics YOLO[CP/OL]. (2023)[2026-06-18]. https://github.com/ultralytics/ultralytics.",
  )
  文献(
    9,
  ...
  文献(
    13,
    "Intel Corporation. OpenVINO Toolkit: Open Visual Inference and Neural Network Optimization[CP/OL]. [2026-06-18]. https://github.com/openvinotoolkit/openvino.",
  )
  文献(
    14,
  ...
  文献(15, "Qwen Team. Qwen3 Technical Report[R/OL]. (2025)[2026-06-18]. https://arxiv.org/abs/2505.09388.")
  文献(16, "DeepSeek-AI. DeepSeek-V3 Technical Report[R/OL]. (2024)[2026-06-18]. https://arxiv.org/abs/2412.19437.")
  文献(17, "Qwen Team. Qwen2.5 Technical Report[R/OL]. (2024)[2026-06-18]. https://arxiv.org/abs/2412.15115.")
  文献(
    18,
