...
第五章的诊断智能体在没有可用大模型时会退回确定性模板；算力允许时，则由大模型把结构化诊断结果改写成更容易理解的讲解。为把这一环节放到边缘端本地运行、减少对云端的依赖，本项目采用双教师蒸馏#引[13]训练了一款面向教学问答的小模型，并完成 OpenVINO INT4 导出与初步评测。

蒸馏数据不是让教师模型自由发挥生成的，而是严格建立在系统已经固定好的结构化证据上。项目选用本地部署的 Qwen3-32B#引[14]和云端的 DeepSeek-V3#引[15]作为双教师，让它们在同一份冻结证据上分别作答，把回答内容和可核查的事实对应起来，尽量减少幻觉。候选问题围绕典型电路、常见故障和学生常问的问题自动扩展，再结合人工与规则过滤掉不贴近课堂场景的样本；教师回答也不会直接拿来训练，只保留有明确依据、没有被降级为“证据不足”、长度合适且表达基本一致的样本。最终从 4990 条候选教师输出中保留 3466 条高质量样本，并构建为 3450 条正式 SFT 数据（候选留存率约 69.1%）。学生模型以 Qwen2.5-1.5B-Instruct#引[16]为基座，经 LoRA#引[17]微调并合并权重后，再进行 INT4 量化#引[18]并导出为 OpenVINO 部署模型：合并后约 3.1 GB、INT4 约 0.88 GB，拷贝到板端即可运行，全程无需外网。

#figure(
  image("board_data/distil_record.png", width: 100%),
  caption: [基于冻结证据的双教师蒸馏流程],
) <fig:distill-record>

如果用简单流程来概括，这部分蒸馏主要分五步：先整理课堂里常见的问题；再运行正式诊断链路，把这道题对应的场景、错误码、故障案例、工具结果和风险等级固定下来；然后让 Qwen3-32B 与 DeepSeek-V3 只根据这份冻结证据作答，危险场景要先提醒断电；接着把没有依据、过短或者已被降级为“证据不足”的回答筛掉；最后把“问题 + 冻结证据 + 合格教师答案”整理成 SFT 数据，用来微调 1.5B 学生模型，并在合并权重后导出 INT4 OpenVINO 模型。以“UA741 反相放大输出钉在 +13 V”为例，冻结证据里会保留 `FLOATING_PIN`、`floating_connection` 和“负电源未连接”等关键信息，因此教师回答也必须围绕这些事实展开。
...
和常见那种强调“把大模型能力搬给小模型”的思路不同，本项目更关注让小模型在课堂场景里答得稳、答得准、答得安全。第一，教师答案必须基于冻结证据，尽量减少幻觉；第二，训练时和板端部署时用的是同一套检索规则和场景白名单，避免训练一套、部署另一套；第三，学生模型后面还有确定性校验器兜底，只要回答越过结构化事实边界，就会被拦下来。因此，这里更像是在训练一个“会看证据说话”的端侧教学解释模型。
...
在同一题集、同一约束和同一评委下，对比本地 `FP` merged 模型和部署版 `INT4` 模型，结果如#ref(<fig:p05-judge-quality>)所示。LLM-Judge 综合得分上，`INT4` 为 3.37/5，`FP` 为 3.33/5；正确性上，`INT4` 为 3.90/5，`FP` 为 3.87/5；通过率上，`INT4` 为 90.0%，`FP` 为 83.3%。按意图细分，`INT4` 在实验指导和诊断排查上略好，`FP` 只在混合问答上稍高一点。整体来看，量化后没有出现明显质量下降，`INT4` 部署版反而略优。
...
项目进一步用同一套 30 题，对未蒸馏基座、蒸馏 `FP`、蒸馏 `INT4` 和教师上限四个模型做了横向对比，结果如#ref(<fig:p0-four-judge>)所示。综合得分呈现清晰排序：教师（3.93）> `INT4`（3.37）> `FP`（3.33）> 基座（3.23），说明蒸馏后学生模型整体优于未蒸馏基座，而量化也没有把效果拉低。进一步看细项，教学启发性仍然是当前短板，这和目前回答长度较短有关；但在接地安全性上，蒸馏模型相比基座有稳定提升。

和原来的简单人工打分相比，LLM-Judge 的自动评分能更清楚地看出问题出在哪一类题、哪一个维度。就目前结果看，后续更值得继续优化的是诊断排查类题目，以及教学启发性这一项。
