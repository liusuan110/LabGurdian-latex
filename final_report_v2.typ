...
作为对照，同一模型如果改在板端 CPU 上运行，虽然吞吐接近，但整体封装平均功率会升到约 43 W，约为 iGPU 的 4.7 倍。这说明把解释生成放到 iGPU 上更省电，而 CPU 更适合留给拓扑分析、检索和服务控制等任务；同时，把模型蒸馏到 1.5B 也是它能落进边缘端内存和功耗预算的重要前提。
