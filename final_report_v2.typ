...
诊断报告还包含面向交互端的高亮协议，把每条诊断项的证据引用转换为可直接绘制的高亮目标（元件框、引脚点、当前/目标/候选孔位）；智能体生成解释时复用同一份协议，使自然语言说明与界面定位指向同一份事实。端到端诊断示例见#ref(<fig:e2e>)。
...
#figure(
  ...
  caption: [端到端诊断示例：(a) 元件检测框与引脚关键点；(b) 重构的结构化网表；(c) 诊断项与证据高亮——“神经感知 → 符号判断 → 可解释定位”端到端贯通],
) <fig:e2e>
