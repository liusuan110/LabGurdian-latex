...
训练优化方式也尽量保持工程可落地。学生模型训练使用 AdamW 优化器，学习率设为 5e-5，并采用 cosine 学习率调度、warmup、weight decay 和梯度裁剪；训练过程使用 bf16，并通过梯度累积控制显存占用。训练完成后得到的是 LoRA 权重，再与基座模型做权重合并，最后导出为 INT4 OpenVINO 部署模型。这样一来，报告里既能说明蒸馏的网络架构，也能把训练和部署之间的连接关系交代清楚。
