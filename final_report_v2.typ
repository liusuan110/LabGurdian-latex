...
围绕上述需求，本项目的主要工作如下。（1）提出面向真实拍摄照片的引脚级电气网表重构方法，以关键点检测与孔位吸附替代元件级目标检测，将识别粒度下沉至引脚与孔位；（2）以确定性图比对承担对错判断、智能体仅在受约束证据内生成解释，并由回答合规校验拦截无依据陈述，构成“神经—符号”的可审计诊断链；（3）面向边缘部署，采用双教师蒸馏与 INT4 量化得到端侧教学解释模型，并在 Intel DK-2500 上以 NPU、iGPU、CPU 异构分工实现本地推理；（4）通过三组消融与端侧实测，分别考察检索约束、模型蒸馏与量化的作用。
...
#figure(
  [#set text(size: 9pt)
    #diagram(
      ...
      node((2.95, 4), [回答合规\ 校验], stroke: 1.3pt + black, name: <c2>),
  ...
  caption: [神经—符号的三链路，感知、符号两链“生产”可审计的结构化事实，安全敏感的对错判定只发生在符号判定链（VF2 确定性图比对）；以“事实生产 / 话术生成”为解释边界，第三链才由受约束小模型把事实改写为教学语言，并经回答合规校验拦截无依据陈述（校验失败即打回重写）],
) <fig:three-chains>

== 总体架构

总体架构要回答的核心问题是，如何既用上大模型的语言能力，又不让它参与安全敏感的对错判定。LabGuardian 的答案是以分层把“事实生产”与“话术生成”在结构上隔开。系统采用交互端与服务端分离，交互端负责图片上传、参数与参考电路选择、结果可视化、人工修正与智能体对话；服务端承接协议入口、视觉与拓扑流水线、领域规则验证、知识检索、智能体编排与遥测推流。整体可概括为“交互层—API 服务层—流水线事实层—领域规则层—Agent 与知识解释层—基础支撑层”六个层次。其中流水线事实层与领域规则层对应感知与符号两链，以确定性算法产出可审计的结构化事实；Agent 与知识解释层对应受约束的情景解释链，才把事实改写为教学语言，三链路与各层次的对应如#ref(<fig:three-chains>)所示。由此“事实生产”与“话术生成”相互分开，下游的自然语言解释只能以上游可核验的结构化事实为依据。

== 主业务流程

一次完整诊断从交互端上传图片开始。用户选定参考电路与阈值参数后，图片提交至同步诊断接口，编排器依次执行从组件检测到语义分析的各环节，服务层把结果封装为统一诊断结果并同步到课堂状态。交互端展示事实链与诊断报告后，自动向智能体请求基于运行证据的诊断解释与下一步建议。整体诊断流程如#ref(<fig:three-chains>)所示，沿“感知产出网表、符号判定产出错误码、受约束模型产出讲解”逐链推进。
...
本文提出并实现了 LabGuardian，一个面向电子实验教学、运行于英特尔边缘平台的神经—符号电路诊断系统。系统以真实拍摄的面包板照片为起点，经关键点检测与孔位吸附重构引脚级电气网表，由确定性图比对承担可审计的对错判断，再由受约束的智能体在回答合规校验约束下生成有依据的解释。教学解释模型经双教师蒸馏与 INT4 量化部署于 DK-2500，以 NPU、iGPU、CPU 异构分工实现本地推理。板端实测表明，系统可在 8 GB 内存预算内完成视觉识别与端侧解释，延迟、吞吐与功耗满足课堂实时使用；检索约束使同一小模型从无法指认故障转为正确指出故障，而教学解释的事实正确性由回答合规校验保证。
